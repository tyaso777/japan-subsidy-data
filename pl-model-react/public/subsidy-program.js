window.PL_SUBSIDY_PROGRAM = {
  ...
  definitions: {
    ...
    managementMetrics: [
      ...
      { id: 'company-value-added-growth', label: '全社付加価値増加率', enabled: true, scope: 'company', timePoints: [{ id: 'A', anchor: { type: 'specialYear', specialYearId: 'base' }, offset: 0 }, { id: 'B', anchor: { type: 'periodEnd', periodId: 'report' }, offset: 0 }], formula: '(([付加価値額][B] / [付加価値額][A]) ^ (1 / YEARS(A, B)) - 1) * 100', outputUnit: '% / 年', target: 35, direction: 'min', optimization: 'adjustable' },
      { id: 'company-productivity-growth', label: '労働生産性成長率', enabled: true, scope: 'company', timePoints: [{ id: 'A', anchor: { type: 'specialYear', specialYearId: 'base' }, offset: 0 }, { id: 'B', anchor: { type: 'periodEnd', periodId: 'report' }, offset: 0 }], formula: '(([労働生産性][B] / [労働生産性][A]) ^ (1 / YEARS(A, B)) - 1) * 100', outputUnit: '% / 年', target: 23.7, direction: 'min', optimization: 'adjustable' },
      { id: 'latest-sales-investment-ratio', label: '売上高投資比率（最新決算期における比率）', enabled: true, scope: 'company', timePoints: [{ id: 'A', anchor: { type: 'historicalEnd' }, offset: 0 }], formula: '0', outputUnit: '%', target: 54.7, direction: 'min', optimization: 'fixed', requiresActualInput: true },
      ...
      { id: 'employee-payroll-growth', label: '給与支給総額の増加率', enabled: true, scope: 'company', timePoints: [{ id: 'A', anchor: { type: 'specialYear', specialYearId: 'base' }, offset: 0 }, { id: 'B', anchor: { type: 'periodEnd', periodId: 'report' }, offset: 0 }], formula: '(([従業員給与総額][B] / [従業員給与総額][A]) ^ (1 / YEARS(A, B)) - 1) * 100', outputUnit: '% / 年', target: 17.4, direction: 'min', optimization: 'adjustable' },
      ...
      { id: 'latest-equity-ratio', label: '最新決算期の自己資本比率', enabled: true, scope: 'company', timePoints: [{ id: 'A', anchor: { type: 'historicalEnd' }, offset: 0 }], formula: '0', outputUnit: '%', target: 43.8, direction: 'min', optimization: 'fixed', requiresActualInput: true },
      { id: 'local-benchmark-score', label: 'ローカルベンチマークの得点', enabled: true, scope: 'company', timePoints: [{ id: 'A', anchor: { type: 'historicalEnd' }, offset: 0 }], formula: '0', outputUnit: '点', target: 22.3, direction: 'min', optimization: 'fixed', requiresActualInput: true },
      { id: 'latest-roa', label: '最新決算期のROA', enabled: true, scope: 'company', timePoints: [{ id: 'A', anchor: { type: 'historicalEnd' }, offset: 0 }], formula: '0', outputUnit: '%', target: 5.1, direction: 'min', optimization: 'fixed', requiresActualInput: true }
    ]
  },
  timeline: {
