...
添付したB/S・P/L資料、このプロンプトで指定する取込対象年度、および末尾に埋め込まれたJSON Schemaを読み取り、このシミュレーター用の過去実績JSONを1件作成してください。

## 出力ファイル

- 出力ファイル名は `actuals-import.json` としてください。
...
- P/Lは資料の性質に応じて、次のいずれか一方の入力方式を選んでください。
  - 全社P/Lが資料に記載されている場合：`profitAndLoss.inputMode`を`company`とし、`company`と`subsidy`を出力してください。`base`は出力しません。アプリが全社P/L－補助事業P/Lでベース事業P/Lを算出します。
  - ベース事業P/Lが明示されている場合：`profitAndLoss.inputMode`を`base`とし、`base`と`subsidy`を出力してください。`company`は出力しません。アプリがベース事業P/L＋補助事業P/Lで全社P/Lを算出します。
- 全社P/Lしかない資料をベース事業P/Lとして転記しないでください。補助事業の区分資料がなければ`subsidy`を空配列にし、`notes`に記録してください。
- ベース事業と補助事業の金額を推測配分しないでください。
...
```json
{
  ...
  "notes": ["資料は全社P/Lであり、補助事業の区分資料がないためsubsidyは空配列"]
}
```
