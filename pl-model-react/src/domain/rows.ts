...
const baseHistoricalPlRows: FinancialRow<HistoricalPlCalculated>[] = [
  ...
  { code: 'S2', label: '原価内減価償却費の売上高比率', valueKind: 'percent', indent: 2, calculated: true, supplementary: true, value: (row) => row.sales ? row.cogsDepreciation / row.sales * 100 : 0 },
  ...
  { code: 'S3', label: '販管費内減価償却費の売上高比率', valueKind: 'percent', indent: 2, calculated: true, supplementary: true, value: (row) => row.sales ? row.sgaDepreciation / row.sales * 100 : 0 },
