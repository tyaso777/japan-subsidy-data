...
function AppContent() {
  ...
  return <TooltipProvider><div data-testid="app-shell" className="mx-auto mt-3.5 mb-10 w-[calc(100%-28px)] max-w-[1900px]" style={{ '--app-toolbar-sticky-bottom': `${stickyStackOffset(0, appToolbar.height)}px` } as CSSProperties}>
    ...
    <div ref={appToolbar.ref} data-testid="app-toolbar" className="sticky top-0 z-50 flex items-center justify-end gap-2 border-x border-b border-line bg-surface px-3 py-2 shadow-sm max-[1100px]:overflow-x-auto"><nav aria-label="主要画面" className="flex gap-0.5 rounded-lg bg-[#e8e6df] p-1">{pages.map((item) => {
