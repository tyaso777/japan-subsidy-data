...
function SettingRow({ series, periodId, periodLabel, unit, variationOpen, onForecastInputChange, readOnly = false }: { series: ForecastSeries; periodId: string; periodLabel: string; unit: MoneyDisplayUnit; variationOpen: boolean; onForecastInputChange?: () => void; readOnly?: boolean }) {
  ...
  const updateSetting = (patch: Partial<Pick<ForecastPeriod, 'annualGrowthRate' | 'startValue' | 'startAdjustment' | 'range'>>) => {
...
export function ForecastPage({ onOpenLogicMap }: { onOpenLogicMap?: (code: string) => void }) {
  ...
  const periodDisplayStartYear = (periodId: string, fallback: number) => model.series[0]?.periods.find((period) => period.id === periodId)?.boundaryYear ?? fallback;
  ...
  const fixedSettingIds = new Set(settings.filter((series) => series.changePolicy === 'fixed' || segments.every((segment) => {
    const period = series.periods.find((candidate) => candidate.id === segment.id);
    return Boolean(period && isForecastRangeLocked(period.range ?? defaultForecastRange(series.projectionMode)));
  })).map((series) => series.id));
  const visibleSettings = showFixedSettings ? settings : settings.filter((series) => !fixedSettingIds.has(series.id));
  ...
  return <main className="mt-3 grid gap-3">
    ...
    <div data-testid="forecast-layout" data-settings-visible={settingsVisible} data-metrics-visible={metricsVisible} className={cn('grid items-start gap-3', forecastLayoutColumns)}>
      ...
      >
        <div data-testid="forecast-period-grid" className="grid min-w-0 gap-2" style={{ gridTemplateColumns: `repeat(${segments.length}, minmax(${settingsPeriodMinWidth(segments.length, variationOpen)}, 1fr))` }}>{segments.map((period, segmentIndex) => { const definitionLabel = program.definitions.periods.find((definition) => definition.id === period.definitionId)?.label ?? period.definitionId; const siblings = segments.filter((candidate) => candidate.definitionId === period.definitionId); const label = siblings.length > 1 ? `${definitionLabel}${siblings.indexOf(period) + 1}` : definitionLabel; const displayStartYear = periodDisplayStartYear(period.id, period.startYear); return <section data-testid="forecast-period-column" key={period.id} className="min-w-0 bg-background"><StickySurface data-testid="forecast-period-header" stickyTop="0px" layer="panel" className="flex min-h-10 items-center justify-between gap-2 border-t-[3px] border-navy px-1.5 py-1 shadow-sm"><span className="flex min-w-0 items-center gap-2"><strong className="min-w-0 text-sm leading-tight">{label}</strong><span data-testid="forecast-period-years" className="shrink-0 whitespace-nowrap text-[10px] tabular-nums text-muted-foreground">{displayStartYear}–{period.endYear}</span></span>{segmentIndex > 0 && segments[segmentIndex - 1].definitionId === period.definitionId && <Button variant="ghost" size="sm" className="h-6 shrink-0 px-1.5 text-[9px]" aria-label={`${period.startYear}年の期間分割を解除`} onClick={() => mergeForecastPeriod(period.id)}>解除</Button>}</StickySurface>{visibleSettings.map((series) => <SettingRow key={series.id} series={series} periodId={period.id} periodLabel={label} unit={unit} variationOpen={variationOpen} onForecastInputChange={optimization.invalidateProposal} readOnly={false} />)}</section>; })}</div>
      </StickyPanel>}
