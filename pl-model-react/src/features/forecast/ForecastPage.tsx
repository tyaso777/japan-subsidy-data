  ...
  return <fieldset data-testid={`forecast-setting-row-${series.id}-${period.id}`} disabled={readOnly} className="m-0 min-w-0 border-0 border-t border-line px-1.5 py-1 [container-type:inline-size] first:border-t-0 disabled:opacity-55">
    ...
    <div data-testid="forecast-setting-controls" className="forecast-setting-controls grid grid-cols-1 items-center gap-1.5">
      <div data-testid="forecast-level-slider-group" className="grid min-w-0 grid-cols-[38px_minmax(44px,1fr)_38px] items-center gap-1">
        <NumberInput disabled={series.changePolicy === 'fixed'} aria-label={`${periodLabel} ${series.label} 最小値`} value={displayRange.min} step="0.01" className="h-5 px-1 text-right text-[9px]" onValueChange={(inputValue) => { const min = normalizeRate(inputValue); update(series.id, period.id, { range: { min, max: displayRange.max } }); }} />
        ...
        <NumberInput disabled={series.changePolicy === 'fixed'} aria-label={`${periodLabel} ${series.label} 最大値`} value={displayRange.max} step="0.01" className="h-5 px-1 text-right text-[9px]" onValueChange={(inputValue) => { const max = normalizeRate(inputValue); update(series.id, period.id, { range: { min: displayRange.min, max } }); }} />
      </div>
      {variationOpen && <div data-testid="forecast-start-adjustment-group" className="grid grid-cols-2 gap-1 border-t border-dashed border-line pt-1"><label className="min-w-0 text-[7px] leading-none text-muted-foreground">開始時固定値<NumberInput aria-label={`${periodLabel} ${series.label} 開始時固定値`} title="未設定時は前年から通常計算" value={startValue} emptyValue={null} step="0.01" className="mt-0.5 h-5 w-full px-1 text-right text-[9px]" onEmpty={() => update(series.id, period.id, { startValue: null })} onEditingStart={begin} onEditingEnd={commit} onValueChange={(inputValue) => update(series.id, period.id, { startValue: fromDisplayFinancialValue(inputValue, series.valueKind, unit) })} /></label><label className="min-w-0 text-[7px] leading-none text-muted-foreground">開始時増減<NumberInput aria-label={`${periodLabel} ${series.label} 開始時増減`} title="固定値または通常計算値へ加算" value={adjustment} step="0.01" className="mt-0.5 h-5 w-full px-1 text-right text-[9px]" onEditingStart={begin} onEditingEnd={commit} onValueChange={(inputValue) => update(series.id, period.id, { startAdjustment: fromDisplayFinancialValue(inputValue, series.valueKind, unit) })} /></label></div>}
    </div>
