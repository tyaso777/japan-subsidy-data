...
import { formatFinancialValue, fromDisplayFinancialValue, moneyUnitLabel, toDisplayFinancialValue, type MoneyDisplayUnit, type ValueKind } from '../../domain/value-units';
...
function SettingRow({ series, periodId, periodLabel, unit, readOnly = false }: { series: ForecastSeries; periodId: string; periodLabel: string; unit: MoneyDisplayUnit; readOnly?: boolean }) {
  ...
  const adjustment = toDisplayFinancialValue(period.startAdjustment, series.valueKind, unit);
  ...
  const fixed = toDisplayFinancialValue(layers.fixedAnnualIncrement, series.valueKind, unit);
  const step = toDisplayFinancialValue(layers.steps[period.startYear] ?? 0, series.valueKind, unit);
  const spot = toDisplayFinancialValue(layers.spots[period.startYear] ?? 0, series.valueKind, unit);
  return <fieldset data-testid={`forecast-setting-row-${series.id}-${period.id}`} disabled={readOnly} className="m-0 min-w-0 border-0 border-t border-line px-1.5 py-1.5 first:border-t-0 disabled:opacity-55">
    ...
    <div className="grid grid-cols-[minmax(0,1fr)_54px] items-end gap-1.5">
      <div className="min-w-0">
        <div className="mb-0.5 flex items-center justify-center gap-0.5"><Input data-position="slider-top" aria-label={`${periodLabel} ${series.label} 年間変化`} type="number" value={period.annualGrowthRate} step="0.1" className="h-6 w-14 px-1 text-right text-[10px] font-bold text-orange" onFocus={begin} onBlur={commit} onChange={(event) => update(series.id, period.id, { annualGrowthRate: Number(event.target.value), range: { min: Math.min(range.min, Number(event.target.value)), max: Math.max(range.max, Number(event.target.value)) } })} /><small className="whitespace-nowrap text-[8px] text-orange">{linear ? 'pt/年' : '%/年'}</small></div>
        <div data-testid="forecast-level-slider-group" className="grid grid-cols-[38px_minmax(44px,1fr)_38px] items-center gap-1">
          <Input aria-label={`${periodLabel} ${series.label} 最小値`} type="number" value={range.min} step="0.1" className="h-6 px-1 text-right text-[9px]" onChange={(event) => { const min = Number(event.target.value); update(series.id, period.id, { range: { min, max: range.max } }); }} />
          <input aria-label={`${periodLabel} ${series.label} 水準`} type="range" min={range.min} max={range.max} step="0.1" value={Math.max(range.min, Math.min(range.max, period.annualGrowthRate))} disabled={range.min === range.max} onPointerDown={begin} onPointerUp={commit} onChange={(event) => update(series.id, period.id, { annualGrowthRate: Number(event.target.value) })} className="w-full accent-[#c75b24]" />
          <Input aria-label={`${periodLabel} ${series.label} 最大値`} type="number" value={range.max} step="0.1" className="h-6 px-1 text-right text-[9px]" onChange={(event) => { const max = Number(event.target.value); update(series.id, period.id, { range: { min: range.min, max } }); }} />
        </div>
      </div>
  ...
  return <main className="mt-3 grid gap-3">
    ...
    <div data-testid="forecast-layout" className="grid grid-cols-[clamp(360px,26vw,500px)_minmax(0,1fr)_clamp(320px,22vw,420px)] items-start gap-3">
      <aside data-testid="forecast-settings-panel" className="sticky top-3 max-h-[calc(100vh-24px)] overflow-y-auto border border-line bg-surface p-2.5">
        <div className="mb-2 flex items-center justify-between"><div><h3 className="m-0 text-base font-bold">水準設定</h3><p className="m-0 text-[10px] text-muted-foreground">{scope === 'company' ? '全社合算ではベース事業の水準を表示' : scopeLabels[scope]}・右端は開始時増減</p></div><Badge variant="outline">内部：円</Badge></div>
