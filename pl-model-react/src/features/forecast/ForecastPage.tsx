...
import { BarChart3, CheckCircle2, ChevronDown, Eye, EyeOff, Sparkles, SlidersHorizontal, Table2, TriangleAlert, Workflow } from 'lucide-react';
...
import { buildForecastPl, fitForecastPlCell, type ForecastPeriod, type ForecastSeries } from '../../domain/forecast-engine';
...
import { defaultForecastRange, isForecastPeriodOptimizationFixed } from '../../domain/forecast-range';
...
function SettingRow({ series, periodId, periodLabel, unit, variationOpen, onForecastInputChange, readOnly = false }: { series: ForecastSeries; periodId: string; periodLabel: string; unit: MoneyDisplayUnit; variationOpen: boolean; onForecastInputChange?: () => void; readOnly?: boolean }) {
  ...
  const updateSetting = (patch: Partial<Pick<ForecastPeriod, 'annualGrowthRate' | 'startValue' | 'startAdjustment' | 'range' | 'optimizationFixed'>>) => {
    onForecastInputChange?.();
    update(series.id, period.id, patch);
  };
  ...
  const optimizationFixed = series.changePolicy === 'fixed' || period.optimizationFixed === true;
  const displayRange = {
    min: roundFinancialInputValue(range.min, rateKind, unit),
    max: roundFinancialInputValue(range.max, rateKind, unit),
  };
  const sliderDisabled = series.changePolicy === 'fixed' || displayRange.min === displayRange.max;
  const sliderVisualState = sliderDisabled ? 'disabled' : optimizationFixed ? 'optimization-fixed' : 'adjustable';
  const sliderValue = Math.max(displayRange.min, Math.min(displayRange.max, annualGrowthRate));
  const sliderSpan = displayRange.max - displayRange.min;
  const sliderProgress = sliderSpan > 0 ? ((sliderValue - displayRange.min) / sliderSpan) * 100 : 0;
  const sliderStyle = {
    '--forecast-slider-progress': `${sliderProgress}%`,
    '--forecast-slider-fill': sliderDisabled ? '#c9ced3' : optimizationFixed ? '#98a2b3' : '#c75b24',
    '--forecast-slider-rest': sliderDisabled ? '#e8ebed' : optimizationFixed ? '#e1e5e8' : '#d9dee2',
    '--forecast-slider-thumb': sliderDisabled ? '#c9ced3' : optimizationFixed ? '#8793a1' : '#c75b24',
  } as CSSProperties;
  ...
  return <fieldset data-testid={`forecast-setting-row-${series.id}-${period.id}`} data-optimization-fixed={optimizationFixed ? 'true' : 'false'} disabled={readOnly} className={cn('m-0 min-w-0 border-0 border-t border-line px-1.5 py-1 [container-type:inline-size] first:border-t-0 disabled:opacity-55', optimizationFixed && 'bg-[#f1f3f3] text-muted-foreground')}>
    ...
    <div data-testid="forecast-setting-controls" className="forecast-setting-controls grid grid-cols-1 items-center gap-1.5">
      <div data-testid="forecast-level-slider-group" className="grid min-w-0 grid-cols-[38px_minmax(44px,1fr)_38px] items-center gap-1">
        ...
        <input aria-label={`${periodLabel} ${series.label} 水準`} data-visual-state={sliderVisualState} type="range" min={displayRange.min} max={displayRange.max} step="0.01" value={sliderValue} disabled={sliderDisabled} style={sliderStyle} onPointerDown={begin} onPointerUp={commit} onChange={(event) => updateSetting({ annualGrowthRate: normalizeRate(Number(event.target.value)) })} className={cn('forecast-setting-slider w-full', sliderDisabled ? 'forecast-setting-slider--disabled cursor-not-allowed' : optimizationFixed ? 'forecast-setting-slider--optimization-fixed cursor-ew-resize' : 'forecast-setting-slider--adjustable cursor-ew-resize')} />
...
export function ForecastPage({ onOpenLogicMap, initialView = 'chart', renderWorkspace = true }: { onOpenLogicMap?: (code: string) => void; initialView?: ForecastView; renderWorkspace?: boolean }) {
  ...
  const fixedSettingKeys = new Set(settings.flatMap((series) => segments.flatMap((segment) => {
    const period = series.periods.find((candidate) => candidate.id === segment.id);
    return series.changePolicy === 'fixed' || (period && isForecastPeriodOptimizationFixed(period, series.projectionMode)) ? [`${series.id}:${segment.id}`] : [];
  })));
  const fixedSettingIds = fixedSettingKeys;
  const periodDividerYears = segments.slice(1).map((period) => period.startYear);
  ...
  const renderDetailCharts = (chartScope: Scope) => {
    ...
    return charts.map((chart) => {
      ...
      return <MultiLineChart key={`${chartScope}-${chart.title}`} title={chart.title} subtitle={chart.subtitle} contextLabel={chartScope === 'base' ? undefined : scopeLabels[chartScope]} years={timeline.years} lines={lines} boundaries={periodDividerYears} kind={chart.kind} unit={unit} editableFromYear={model.series[0].baseYear + 1} specialYearLabels={specialYearLabels} />;
    });
  };
  const renderComparisonCharts = () => comparisonCharts.map(([title, field, kind]) => <MultiLineChart key={title} title={title} subtitle="" contextLabel="事業比較" years={company.years} lines={comparison(field)} boundaries={periodDividerYears} kind={kind} unit={unit} editableFromYear={model.series[0].baseYear + 1} specialYearLabels={specialYearLabels} />);
  ...
  return <main className="mt-3 grid gap-3">
    ...
    <div data-testid="forecast-layout" data-settings-visible={settingsVisible} data-metrics-visible={metricsVisible} className={cn('grid items-start gap-3', forecastLayoutColumns)}>
      ...
      >
        <div data-testid="forecast-period-grid" className="grid min-w-0 gap-2" style={{ gridTemplateColumns: `repeat(${segments.length}, minmax(${settingsPeriodMinWidth(segments.length, variationOpen)}, 1fr))` }}>{segments.map((period, segmentIndex) => { const definitionLabel = program.definitions.periods.find((definition) => definition.id === period.definitionId)?.label ?? period.definitionId; const siblings = segments.filter((candidate) => candidate.definitionId === period.definitionId); const label = siblings.length > 1 ? `${definitionLabel}${siblings.indexOf(period) + 1}` : definitionLabel; const periodSettings = showFixedSettings ? settings : settings.filter((series) => !fixedSettingKeys.has(`${series.id}:${period.id}`)); return <section data-testid="forecast-period-column" key={period.id} className="min-w-0 bg-background"><StickySurface data-testid="forecast-period-header" stickyTop="0px" layer="panel" className="flex min-h-10 items-center justify-between gap-2 border-t-[3px] border-navy px-1.5 py-1 shadow-sm"><span className="flex min-w-0 items-center gap-2"><strong className="min-w-0 text-sm leading-tight">{label}</strong><span data-testid="forecast-period-years" className="shrink-0 whitespace-nowrap text-[10px] tabular-nums text-muted-foreground">{period.startYear}–{period.endYear}</span></span>{segmentIndex > 0 && segments[segmentIndex - 1].definitionId === period.definitionId && <Button variant="ghost" size="sm" className="h-6 shrink-0 px-1.5 text-[9px]" aria-label={`${period.startYear}年の期間分割を解除`} onClick={() => mergeForecastPeriod(period.id)}>解除</Button>}</StickySurface>{periodSettings.map((series) => <SettingRow key={series.id} series={series} periodId={period.id} periodLabel={label} unit={unit} variationOpen={variationOpen} onForecastInputChange={optimization.invalidateProposal} readOnly={false} />)}</section>; })}</div>
      </StickyPanel></>}
