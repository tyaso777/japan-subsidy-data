...
function SettingRow({ series, periodId, periodLabel, unit, variationOpen, readOnly = false }: { series: ForecastSeries; periodId: string; periodLabel: string; unit: MoneyDisplayUnit; variationOpen: boolean; readOnly?: boolean }) {
  ...
  return <fieldset data-testid={`forecast-setting-row-${series.id}-${period.id}`} disabled={readOnly} className="m-0 min-w-0 border-0 border-t border-line px-1.5 py-1 [container-type:inline-size] first:border-t-0 disabled:opacity-55">
    ...
    {variationOpen && showEffects && <div className="mt-2 grid grid-cols-2 gap-1 rounded border border-line bg-surface p-1.5"><label className="text-[8px] text-muted-foreground">毎年固定増減<NumberInput aria-label={`${periodLabel} ${series.label} 毎年固定増減`} value={fixed} className="mt-0.5 h-6 px-1 text-right text-[9px]" onEmptyChange={() => updateLayer(series.id, period.id, { fixedAnnualIncrement: 0 })} onValueChange={(inputValue) => updateLayer(series.id, period.id, { fixedAnnualIncrement: fromDisplayFinancialValue(inputValue, series.valueKind, unit) })} /></label><label className="text-[8px] text-muted-foreground">成長加速度<NumberInput aria-label={`${periodLabel} ${series.label} 成長加速度`} value={acceleration} className="mt-0.5 h-6 px-1 text-right text-[9px]" onEmptyChange={() => updateLayer(series.id, period.id, { acceleration: 0 })} onValueChange={(inputValue) => updateLayer(series.id, period.id, { acceleration: inputValue })} /></label><label className="text-[8px] text-muted-foreground">単年以降増減<NumberInput aria-label={`${periodLabel} ${series.label} 単年増減`} value={step} className="mt-0.5 h-6 px-1 text-right text-[9px]" onEmptyChange={() => updateLayer(series.id, period.id, { steps: { [period.startYear]: 0 } })} onValueChange={(inputValue) => updateLayer(series.id, period.id, { steps: { [period.startYear]: fromDisplayFinancialValue(inputValue, series.valueKind, unit) } })} /></label><label className="text-[8px] text-muted-foreground">当年のみ増減<NumberInput aria-label={`${periodLabel} ${series.label} 当年のみ増減`} value={spot} className="mt-0.5 h-6 px-1 text-right text-[9px]" onEmptyChange={() => updateLayer(series.id, period.id, { spots: { [period.startYear]: 0 } })} onValueChange={(inputValue) => updateLayer(series.id, period.id, { spots: { [period.startYear]: fromDisplayFinancialValue(inputValue, series.valueKind, unit) } })} /></label></div>}
  </fieldset>;
}
...
export function ForecastPage() {
  ...
  return <main className="mt-3 grid gap-3">
    ...
    <div data-testid="forecast-layout" className="grid grid-cols-[clamp(320px,20vw,380px)_minmax(0,1fr)_clamp(250px,15vw,290px)] items-start gap-3">
      <aside ref={settingsPanel.ref} data-testid="forecast-settings-panel" className="sticky top-[var(--forecast-content-sticky-top)] flex max-h-[calc(100vh-var(--forecast-content-sticky-top)-12px)] flex-col overflow-hidden border border-line bg-surface">
        <div data-testid="forecast-settings-sticky-header" className="flex shrink-0 items-center justify-between gap-2 border-b border-line bg-surface px-2.5 py-2.5 shadow-sm"><div><h3 className="m-0 text-base font-bold">水準設定</h3><p className="m-0 text-[10px] text-muted-foreground">{scope === 'company' ? '全社合算ではベース事業の水準を表示' : scopeLabels[scope]}・右端は開始時増減</p></div><span className="flex shrink-0 items-center gap-1"><Button variant="outline" size="sm" className="h-7 gap-1 px-2 text-[9px]" aria-label={variationOpen ? '変動設定を隠す' : '変動設定を表示'} aria-expanded={variationOpen} onClick={() => setVariationOverride(!variationOpen)}><ChevronDown className={cn('transition-transform', !variationOpen && '-rotate-90')} />変動設定</Button><Badge variant="outline">金額単位：{moneyUnitLabel(unit)}</Badge></span></div>
        <div data-testid="forecast-settings-scroll-body" className="min-h-0 overflow-x-hidden overflow-y-auto p-2.5">
      ...
      <section className="min-w-0 border border-line bg-surface p-3">
          <TabsContent value="chart" className="mt-0">
            <div ref={chartDisplayLayer.ref} data-testid="forecast-chart-display-controls" className="sticky top-[var(--forecast-content-sticky-top)] z-30 -mx-3 flex items-center justify-between gap-2 border-b border-line bg-surface px-3 py-2 shadow-sm">
