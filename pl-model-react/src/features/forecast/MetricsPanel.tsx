...
export function MetricsPanel({ company, base, subsidy, optimization }: { company: Timeline; base: Timeline; subsidy: Timeline; optimization: ForecastOptimizationController }) {
  ...
  return <aside data-testid="forecast-metrics-panel" style={{ scrollbarGutter: 'stable' }} className="sticky top-[var(--forecast-content-sticky-top)] min-w-0 max-h-[calc(100vh-var(--forecast-content-sticky-top)-12px)] overflow-x-hidden overflow-y-scroll border border-line bg-surface p-2 pr-3">
    <div data-testid="forecast-metrics-sticky-header" className="sticky top-0 z-20 -mx-2 -mt-2 mb-1 flex items-center justify-between gap-2 border-b border-line bg-surface px-2 py-2 pr-3 shadow-sm"><div><h3 className="m-0 text-base font-bold">経営指標・目標</h3><p className="m-0 text-[10px] text-muted-foreground">制度式で全社・事業別を同時評価</p></div><Button variant={editing ? 'default' : 'outline'} size="sm" onClick={() => setEditing((value) => !value)}>{editing ? '編集完了' : 'すべて編集'}</Button></div>
