  ...
  return <main className="mt-3 grid gap-3">
    ...
    <div className="grid grid-cols-2 items-start gap-3 max-[1100px]:grid-cols-1">
      ...
      <DefinitionCard id="special-years" title="特別年の呼称" description="制度固有の年名を、区間の開始・終了年から相対指定します。" addLabel="特別年を追加" onAdd={addSpecialYear}>
        <div className="grid gap-2">{program.definitions.specialYears.map((year, index) => <article key={year.id} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_100px_36px] items-end gap-2 rounded-md border border-line bg-background p-3"><label className={fieldClass}>呼称<Input aria-label={`特別年${index + 1} 呼称`} value={year.label} onChange={(event) => change((draft) => { draft.definitions.specialYears[index].label = event.target.value; })} /></label><label className={fieldClass}>基準期間<select className={selectClass} value={year.anchor.type === 'historicalEnd' ? 'historicalEnd' : `${year.anchor.type}:${year.anchor.periodId}`} onChange={(event) => change((draft) => { const [type, periodId] = event.target.value.split(':'); draft.definitions.specialYears[index].anchor = type === 'historicalEnd' ? { type: 'historicalEnd' } : { type: type as 'periodStart' | 'periodEnd', periodId }; })}><option value="historicalEnd">過去実績・終了年</option>{program.definitions.periods.flatMap((period) => [<option key={`${period.id}-s`} value={`periodStart:${period.id}`}>{period.label}・開始年</option>, <option key={`${period.id}-e`} value={`periodEnd:${period.id}`}>{period.label}・終了年</option>])}</select></label><label className={fieldClass}>調整年数<NumberInput aria-label={`特別年${index + 1} 調整年数`} value={year.offset} onValueChange={(value) => change((draft) => { draft.definitions.specialYears[index].offset = value; })} /></label><Button variant="ghost" size="icon" aria-label={`${year.label}を削除`} onClick={() => replaceProgram(removeSpecialYearDefinition(program, year.id))}><Trash2 /></Button></article>)}</div>
    ...
    <DefinitionCard id="numeric-definitions" title="共通数値定義" description="複数の経営指標から再利用する値を先に定義します。t・t-1等も式に直接記述できます。" addLabel="共通数値定義を追加" onAdd={() => change((draft) => draft.definitions.commonNumericDefinitions.push({ id: nextId('value', draft.definitions.commonNumericDefinitions.length), label: '新しい数値', formula: '0', outputPoint: 't' }))}>
      <div className="grid grid-cols-2 gap-3 max-[1100px]:grid-cols-1">{program.definitions.commonNumericDefinitions.map((definition, index) => <article key={definition.id} className="grid gap-2 border-t-[3px] border-teal bg-background p-3"><div className="grid grid-cols-[minmax(0,1fr)_36px] gap-2"><Input aria-label={`数値定義${index + 1} 名称`} value={definition.label} onChange={(event) => replaceProgram(renameNumericDefinition(program, definition.id, event.target.value))} /><Button variant="ghost" size="icon" aria-label={`${definition.label}を削除`} onClick={() => change((draft) => { draft.definitions.commonNumericDefinitions.splice(index, 1); })}><Trash2 /></Button></div><Textarea aria-label={`${definition.label} 計算式`} value={definition.formula} onChange={(event) => change((draft) => { draft.definitions.commonNumericDefinitions[index].formula = event.target.value; })} /></article>)}</div>
    </DefinitionCard>
