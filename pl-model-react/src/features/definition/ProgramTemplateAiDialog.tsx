    ...
    <Dialog open={open} onOpenChange={(next) => { setOpen(next); if (!next) reset(); }}>
      <DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>公募要領・指標資料と下記資料をCopilot等へ渡し、生成されたJSONを検証してから制度テンプレートへ反映します。</DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-3 rounded-md border border-line bg-soft/50 p-3">
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outline" size="sm" aria-label="AI作成用プロンプトをコピー" onClick={async () => {
              try { await copyText(programTemplatePrompt); setResourceMessage('プロンプトをコピーしました'); }
              catch { setResourceMessage('プロンプトをコピーできませんでした'); }
            }}><Copy />AI作成用プロンプトをコピー</Button>
            <Button variant="outline" size="sm" aria-label="制度テンプレートSchemaをダウンロード" onClick={() => {
