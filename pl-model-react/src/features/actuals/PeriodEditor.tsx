...
const rowClass = 'grid grid-cols-[minmax(180px,1.2fr)_repeat(2,minmax(140px,.8fr))_110px] items-end gap-3.5 bg-surface px-4.5 py-2.5';
...
export function PeriodEditor({ program, onEndYearChange, onHistoricalBoundaryChange }: Props) {
  ...
  return <section className="border border-line bg-surface">
    ...
    <div className="border-t border-line px-4.5 py-3"><div className="mb-2"><h3 className="m-0 text-sm font-bold">特別年の呼称</h3><p className="mt-0.5 mb-0 text-[10px] text-muted-foreground">制度定義の基準時点を、この会社の期間へ当てはめた結果です。</p></div><div data-testid="special-year-summary" className="grid grid-cols-4 gap-2">{specialYears.map((special) => <article key={special.id} className="border border-dashed border-line bg-background px-3 py-2"><strong className="block text-xs">{special.label}</strong><span className="mt-1 block text-lg font-bold text-teal">{special.year}年</span><small className="block text-[9px] text-muted-foreground">{describeSpecialYearAnchor(program, special.id)}</small></article>)}</div></div>
  </section>;
}
