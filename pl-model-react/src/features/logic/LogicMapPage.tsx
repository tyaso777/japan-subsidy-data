...
export function LogicMapPage() {
  ...
  return <main className="mt-3 grid gap-3">
    <section className="flex items-center justify-between border border-line bg-surface px-5 py-4">
      <div><p className="mb-1 flex items-center gap-1 text-[10px] font-extrabold tracking-[.08em] text-orange"><Braces className="size-3" />FORMULA DEPENDENCY GRAPH</p><h2 className="m-0 text-xl font-bold">共通数値定義・ロジックマップ</h2><p className="mt-1 mb-0 text-xs text-muted-foreground">制度JSONで定義した数式を依存順に並べます。循環参照と未定義参照は読み込み時に検出します。</p></div>
    ...
    <section data-testid="definition-logic-map" className="grid grid-cols-4 gap-3 border border-line bg-surface p-4">
      {ordered.map((definition, index) => {
        ...
        return <article key={definition.id} className="relative flex min-h-52 flex-col border border-line bg-background p-4 shadow-sm">
          {index < ordered.length - 1 && <ArrowRight aria-hidden="true" className="absolute top-1/2 -right-5 z-10 size-6 rounded-full border border-line bg-surface p-1 text-teal" />}
          <div className="flex items-center justify-between gap-2"><Badge variant="outline">計算順 {index + 1}</Badge><code className="text-[10px] text-muted-foreground">[{definition.outputPoint}]</code></div>
          <h3 className="mt-3 mb-1 text-base font-bold">{definition.label}</h3>
          ...
          <div className="mt-auto grid gap-1 pt-3 text-[10px]">
            ...
            {calculated.length > 0 && <><p className="mt-1 mb-0 text-muted-foreground">先に計算する定義</p><div className="flex flex-wrap gap-1">{calculated.map((dependency) => <span key={dependency} className="rounded bg-teal/10 px-1.5 py-0.5 text-teal ring-1 ring-teal/20">{dependency}</span>)}</div></>}
          </div>
        </article>;
      })}
    </section>
    <section data-testid="pl-logic-section" className="grid grid-cols-[minmax(0,1fr)_310px] gap-3 border border-line bg-surface p-4 [&>aside]:top-16">
      <div><div className="mb-3"><h3 className="m-0 text-base font-bold">P/L項目と水準設定のつながり</h3><p className="mt-1 mb-0 text-[11px] text-muted-foreground">P/Lの並びを保ったまま項目を選ぶと、参照元・設定値・影響先を確認できます。</p></div><div data-testid="pl-logic-map" className="grid grid-cols-3 gap-2">{plLogicNodes.map((node) => <button type="button" key={node.code} aria-pressed={node.code === selected.code} onClick={() => setSelectedCode(node.code)} className={`grid min-h-16 grid-cols-[38px_1fr] items-center border px-2 py-2 text-left ${node.code === selected.code ? 'border-teal bg-teal/10' : 'border-line bg-background hover:bg-soft'}`}><small className="text-muted-foreground">{node.code}</small><span className="text-xs font-bold">{node.label}</span></button>)}</div></div>
      <aside data-testid="logic-detail" className="sticky top-3 self-start border-t-[3px] border-orange bg-background p-4"><p className="m-0 text-[10px] font-bold text-orange">PL {selected.code}</p><h3 className="mt-1 mb-2 text-lg font-bold">{selected.label}</h3><code className="block rounded bg-soft p-2 text-[11px] leading-relaxed">{selected.formula}</code><h4 className="mt-4 mb-1 text-xs">参照するPL項目</h4><div className="flex flex-wrap gap-1">{selected.dependsOn.length ? selected.dependsOn.map((code) => <span key={code} className="rounded border border-line bg-surface px-2 py-1 text-[10px]">{labels.get(code)}</span>) : <small className="text-muted-foreground">前年値・外部入力</small>}</div><h4 className="mt-4 mb-1 text-xs">使う設定値</h4><div className="flex flex-wrap gap-1">{selected.settings.length ? selected.settings.map((setting) => <span key={setting} className="rounded bg-orange/10 px-2 py-1 text-[10px] text-orange">{setting}</span>) : <small className="text-muted-foreground">直接の設定なし</small>}</div><h4 className="mt-4 mb-1 text-xs">この項目が影響する先</h4><div className="flex flex-wrap gap-1">{downstream.length ? downstream.map((code) => <span key={code} className="rounded bg-teal/10 px-2 py-1 text-[10px] text-teal">{labels.get(code)}</span>) : <small className="text-muted-foreground">最終出力</small>}</div></aside>
    </section>
  </main>;
}
