...
self.onmessage = (event: MessageEvent<WorkerRequest>) => {
  try {
    ...
    const result = request.kind === 'proposal'
      ? createMetricOptimizationProposal(request.model, request.program, request.baseActuals, request.subsidyActuals, request.actualInputs, request.strategy, { includeExpansionPlan: false })
      : createMetricOptimizationExpansionPlan(request.model, request.initial, request.program, request.baseActuals, request.subsidyActuals, request.actualInputs, request.strategy);
