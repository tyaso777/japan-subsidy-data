    ...
    <div className="overflow-x-auto">
      <table className="w-full table-fixed border-collapse text-[10px] select-none">
        ...
        <tbody>{visibleRows.map((row, rowIndex) => <tr key={row.code} className={cn(row.calculated && 'bg-teal/5', row.supplementary && 'bg-orange/5')}>
          ...
          {separateSubjectColumns ? <><td {...cellInteraction(rowIndex, 0)} className={cn(cellClass, 'border-t border-line px-2 text-left text-[9px] font-medium text-muted-foreground', selectedCell(rowIndex, 0) && 'bg-teal/10 ring-1 ring-inset ring-teal')}>{row.displayCode ?? row.code}</td><th {...cellInteraction(rowIndex, 1)} scope="row" className={cn(cellClass, 'border-t border-line px-2 text-left font-bold', selectedCell(rowIndex, 1) && 'bg-teal/10 ring-1 ring-inset ring-teal')}>{onRowSelect ? <button type="button" className={cn('w-full text-left', row.indent === 1 && 'pl-3.5', row.indent === 2 && 'pl-7')} onClick={() => onRowSelect(row)}>{row.label}</button> : <span className={cn(row.indent === 1 && 'pl-3.5', row.indent === 2 && 'pl-7')}>{row.label}</span>}</th></> : <th {...cellInteraction(rowIndex, 0)} scope="row" className={cn(cellClass, 'border-t border-line px-2 text-left font-bold', selectedCell(rowIndex, 0) && 'bg-teal/10 ring-1 ring-inset ring-teal')}>{onRowSelect ? <button type="button" className="grid w-full grid-cols-[3.25rem_minmax(0,1fr)] items-start text-left" onClick={() => onRowSelect(row)}><span className="text-[9px] font-medium text-muted-foreground">{row.displayCode ?? row.code}</span><span className={cn('min-w-0', row.indent === 1 && 'pl-3.5', row.indent === 2 && 'pl-7')}>{row.label}</span></button> : <span className="grid w-full grid-cols-[3.25rem_minmax(0,1fr)] items-start text-left"><span className="text-[9px] font-medium text-muted-foreground">{row.displayCode ?? row.code}</span><span className={cn('min-w-0', row.indent === 1 && 'pl-3.5', row.indent === 2 && 'pl-7')}>{row.label}</span></span>}</th>}
