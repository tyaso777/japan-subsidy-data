...
export function FinancialTable<T extends object>({ testId, title, prefix = '', years, records, rows, onChange, moneyUnit, onEditStart, onEditEnd, editableFromIndex, onValueChange, yearLabels, onRowSelect }: Props<T>) {
  ...
  return <section className="border border-line bg-surface" data-testid={testId}>
    ...
    <div className="overflow-x-auto">
      <table className="w-full table-fixed border-collapse text-[10px]">
        ...
        <thead><tr><th className="h-8 border-t border-line bg-[#f3f1eb] px-2 py-1 text-left text-muted-foreground">科目</th>{years.map((year, index) => { const role = yearLabels?.[year]; return <th className="h-8 border-t border-line bg-[#f3f1eb] px-1 py-1 text-right text-muted-foreground" key={year}><span className="block">{role?.primary ?? closingLabel(index, years.length)}</span>{role?.secondary && <small className="block text-[8px] text-teal">{role.secondary}</small>}<small className="mt-0.5 block text-[9px]">{year}年</small></th>; })}</tr></thead>
