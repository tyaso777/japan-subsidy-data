...
const compare = (next, legacy) => {
  if (next.requiredViolation + tolerance < legacy.requiredViolation) return "改善";
  if (legacy.requiredViolation + tolerance < next.requiredViolation) return "悪化";
  if (next.hardViolation + tolerance < legacy.hardViolation) return "改善";
  if (legacy.hardViolation + tolerance < next.hardViolation) return "悪化";
  if (next.score + 1e-9 < legacy.score) return "改善";
  if (legacy.score + 1e-9 < next.score) return "悪化";
  return "同等";
};
...
for (const [index, scenario] of cases.entries()) {
  ...
  rows.push({ index: index + 1, scenario, legacy, next, judgement: compare(next, legacy) });
  console.log(`${String(index + 1).padStart(2, "0")}/30 ${rows.at(-1).judgement} ${scenario.name}`);
}

const improved = rows.filter((row) => row.judgement === "改善").length;
const equal = rows.filter((row) => row.judgement === "同等").length;
const worsened = rows.filter((row) => row.judgement === "悪化").length;
...
|---:|---|:---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
${rows.map((row) => `| ${row.index} | ${row.scenario.name} | ${row.judgement} | ${number(row.legacy.requiredViolation)} | ${number(row.next.requiredViolation)} | ${number(row.legacy.hardViolation)} | ${number(row.next.hardViolation)} | ${number(row.legacy.score, 3)} | ${number(row.next.score, 3)} | ${row.legacy.failedTargets.length} | ${row.next.failedTargets.length} | ${number(row.legacy.elapsedMs, 0)} | ${number(row.next.elapsedMs, 0)} |`).join("\n")}
`;

const reportPath = path.join(projectDirectory, "optimizer-benchmark-results.md");
await writeFile(reportPath, report, "utf8");
console.log(`Report: ${reportPath}`);
console.log(`Summary: improved=${improved}, equal=${equal}, worsened=${worsened}`);
