...
function BalanceSheetEditor({ balanceSheets, historical, inputValues, omitUnused, onChange }: { balanceSheets: BalanceSheetPlan[]; historical: YearPlan[]; inputValues: InputValues; omitUnused: boolean; onChange: (yearIndex: number, field: keyof BalanceSheetPlan, value: number | null) => void }) {
  ...
  const visibleRows = omitUnused ? rows.filter((item) => item.code === "1-24") : rows;
...
function CompanyTable({ plan, sourcePlan }: { plan: YearPlan[]; sourcePlan?: YearPlan[] }) {
  ...
  return <OfficialRowsTable title="会社全体にかかる損益計算書・関連計算項目" kicker="ROUND 6 / SECTION 2" pill="2-1～2-36" plan={plan} sourcePlan={sourcePlan} rows={rows} note="2-1～2-20が損益計算書、2-21～2-36が給与・付加価値・人数・EBITDAの関連計算項目です。2-8は2-9＋2-10、2-11は2-12＋2-13、2-23は2-4＋2-14で自動計算します。2-18～2-20・2-27・2-28は第6次様式の入力項目で、将来値は自動予測後に上書きできます。" />;
}
