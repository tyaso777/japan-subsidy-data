      ...
      {view === "targets" && (
        <section className="content-stack">
          ...
          <article className="panel table-panel">
            ...
            <div className="targets-table-wrap"><table className="targets-table"><thead><tr><th>No.</th><th>指標</th>{historicalPlan.map((row) => <th key={row.year}>{row.year}<small>{YEAR_ROLE_LABELS[row.role]}</small></th>)}<th>第6次定義：基準年→事業化報告3年目<small>{targetComparisonYears}年間{adjustedPlan ? "／入力 → 調整案" : "／計画値"}</small></th><th>目標下限<small>{targetComparisonYears}年間</small></th><th>計画上限<small>{targetComparisonYears}年間</small></th><th>扱い</th><th>優先度</th><th>判定</th></tr></thead><tbody>
