...
const projectDetailedInputFields: OtherPlInputField[] = otherPlInputFields
  .filter((item) => item.key !== "cogsDepreciation" && item.key !== "sgaDepreciation")
  .map((item) => ({
    ...item,
...
export default function Home() {
  ...
  const validations = useMemo(() => {
    ...
    const combined = statutoryValidations;
    ...
    <main>
      ...
      {view === "summary" && (
        <section className="page-grid summary-grid">
          ...
          <article className="panel validation-panel">
            ...
            <p className="footnote">業種により適正水準が異なるため、原価率・販管費率・営業利益率・年度変動は一律レンジでは判定しません。過去3期実績や同業比較、設備仕様、顧客別数量を用いて確認してください。</p>
          </article>
        </section>
      )}
      ...
      {view === "future" && (
        <section className="content-stack">
          ...
          <article className="panel table-panel">
            <div className="panel-heading"><div><h2>補助事業期間 → 事業化報告3年目</h2></div><span className="pill blue-pill">7-10合計を年度別入力</span></div>
            {businessSegmentationMode === "split" ? <div className="future-basis-setting"><div><strong>将来PLの入力方式</strong><small>公式様式を直接作るか、事業別の詳細PLを積み上げるかを選びます</small></div><div className="mode-switch" role="group" aria-label="将来PLの入力方式"><button type="button" className={futureInputBasis === "company" ? "active" : ""} aria-pressed={futureInputBasis === "company"} onClick={() => changeFutureInputBasis("company")}>全社PLを入力</button><button type="button" className={futureInputBasis === "other" ? "active" : ""} aria-pressed={futureInputBasis === "other"} onClick={() => changeFutureInputBasis("other")}>ベース事業PLを入力</button></div></div> : <p className="default-note">全社PLへの入力が補助事業へそのまま反映されます。</p>}
            {missingAccountingAssumptions.length > 0 && <div className="default-note accounting-assumption-notice" role="alert"><span>取り込んだ固定値は表示しています。空欄の自動予測と会計計算には、給与・賞与、役員報酬・賞与、研究開発費、営業外損益、特別損益、税率の前提設定が必要です。</span><button type="button" className="default-button" onClick={confirmAndApplyHistoricalDefaults}>過去3期から会計前提を設定</button></div>}
            <div className={missingAccountingAssumptions.length > 0 ? "accounting-assumptions-pending" : undefined}>
              <FutureInputsEditor historical={historicalPlan} autoPlan={autoPlan} effectivePlan={sourcePlan} overrides={forecastOverrides} inputValues={inputValues} futureInputBasis={futureInputBasis} businessSegmentationMode={businessSegmentationMode} drivers={calculationDrivers} onForecastChange={updateForecastOverride} />
            </div>
            <p className="footnote">{businessSegmentationMode === "wholeCompanyAsProject" ? "会社全体＝補助事業として入力し、ベース事業は作成しません。" : "「全社PLを入力」は会社全体と補助事業を入力する方式です。「ベース事業PLを入力」は補助事業とベース事業を積み上げる方式です。"}</p>
          </article>
        ...
        <section className="content-stack">
          ...
          <article className="panel">
            ...
            <div className="wide-table spreadsheet-grid driver-target-table"><table><thead><tr><th rowSpan={2}>調整条件<small>C-1～（Condition）</small></th><th rowSpan={2} className="driver-statutory-heading">制度上の必須条件<small>編集不可</small></th>{historicalPlan.slice(1).map((row) => <th rowSpan={2} className="driver-reference-heading" key={row.year}>{row.year}<small>過去実績・参考値<br />{YEAR_ROLE_LABELS[row.role]}</small></th>)}<th colSpan={2} className="driver-period-heading">設備導入期間<small>最新決算期 → 基準年</small></th><th colSpan={2} className="driver-period-heading">基準年後<small>基準年 → 事業化報告3年目</small></th></tr><tr><th className="driver-bound-heading">下限</th><th className="driver-bound-heading">上限</th><th className="driver-bound-heading">下限</th><th className="driver-bound-heading">上限</th></tr></thead><tbody>
              ...
              <p className="footnote">前期・最新決算期は、計画値ではなく過去実績の参考値です。固定条件は下限・上限を持たないため、期間内の2列を結合した入力欄で表示します。表示されている下限・上限がそのまま最適化の探索範囲であり、別の非表示上限は設けません。制度条件や計算上成立しない値は別途バリデーションします。補助事業の減価償却費は調整水準では生成せず、公式7-10の年度別合計を③で入力し、内訳だけを②のベース事業構成比で計算します。</p>
      ...
      {view === "logic" && (
        <section className="content-stack">
          ...
          <article className="panel formula-panel">
            ...
            <p>許容下限・上限は、過去3期の最小・最大に変動幅の50%（最低1pt、率水準は最低2pt）を加え、技術的な上下限内に収めます。過去実績から決められない投資額・補助金額などは自動変更しません。減価償却費は投資額や耐用年数から作りません。補助事業は公式7-10の年度別合計を入力し、P2-4・P2-14は②の「売上原価に含める割合」で自動計算します。P2-Xの減価償却費は直接入力しません。</p>
          </article>
          <article className="panel formula-panel">
            <h2>近似調整の目的関数</h2>
...
const projectOfficialInputRows: ProjectOfficialInputRow[] = [
  ...
  { code: "7-10", label: "減価償却費（合計）", unit: "千円", get: (s) => s.depreciation, set: (_s, v) => ({ depreciation: v }) },
...
const projectOfficialDisplayRows: ProjectOfficialDisplayRow[] = [
  ...
  { code: "P2-4", label: "売上原価に含まれる減価償却費", unit: "千円", indentLevel: 1, get: (rows, index) => cogsDepreciation(rows[index].project) },
  ...
  { code: "P2-14", label: "販管費に含まれる減価償却費", unit: "千円", indentLevel: 1, get: (rows, index) => sgaDepreciation(rows[index].project) },
  { code: "7-10", label: "減価償却費（合計）", unit: "千円", input: projectInputByCode.get("7-10"), get: (rows, index) => rows[index].project.depreciation },
...
function DetailedProjectInputsTable({ historical, effectivePlan, overrides, omitCalculated, onToggleCalculated, onForecastChange }: {
  ...
  return <div>
    ...
    <p className="footnote">第6次公式Excelに対応する項目は7-1～7-19で表示し、給与・賞与の内訳、減価償却費の区分、経常利益以下など公式様式だけでは不足する項目に限りP2-Xを付けています。公式7-10の減価償却費合計を年度別入力し、P2-4・P2-14は②のベース事業構成比で自動計算します。営業外損益（純額）は「経常利益－営業利益」、特別損益（純額）は「税引前当期純利益－経常利益」で自動計算します。補助事業の経常利益以下が未入力の場合、純額は0として表示します。P2-Xは補助事業の詳細PLを作るための内部管理用番号で、公式Excelへ直接転記する番号ではありません。7-20市場伸び率は「15指標・目標」の固定前提を参照します。</p>
  </div>;
}
      ...
      <div className="wide-table"><table><thead><tr><th>第6次様式項目（金額は{moneyUnit}）</th>{historical.map((row) => <th className="historical-heading" key={row.year}>{row.year}<small>{YEAR_ROLE_LABELS[row.role]}・参照</small></th>)}{futureRows.map((row) => <th key={row.year} className="forecast-heading">{row.year}<small>{YEAR_ROLE_LABELS[row.role]}・空欄は原則自動予測</small></th>)}</tr></thead>
        <tbody>{visibleProjectRows.map((item) => <tr className={!item.input ? "calculated-row" : ""} key={item.code}>
          ...
          {futureRows.map((row) => {
            ...
            const overridden = Object.prototype.hasOwnProperty.call(overrides, key);
            const isMoney = item.unit === "千円" || item.unit === "千円/人";
            return <td key={row.year}>{isMoney ? <MoneyInput className={`forecast-override${overridden ? " is-fixed" : ""}`} value={overridden ? overrides[key] : ""} canonicalPlaceholder={value ?? 0} ariaLabel={`${row.year}年 ${item.label}（${overridden ? "手入力固定値" : "空欄は自動予測"}）`} onCanonicalChange={(next) => onForecastChange(row.year, "project", item.code, next)} /> : <input className={`forecast-override${overridden ? " is-fixed" : ""}`} type="number" step="1" value={overridden ? overrides[key] : ""} placeholder={rawPlaceholder(value ?? 0, item.digits ?? 2)} aria-label={`${row.year}年 ${item.label}（${overridden ? "手入力固定値" : "空欄は自動予測"}）`} onChange={(event) => onForecastChange(row.year, "project", item.code, event.target.value === "" ? null : Number(event.target.value))} />}</td>;
          })}
        </tr>)}</tbody>
      </table></div>
...
function FinancialDiagnostics({ plan, balanceSheets, futureCapex, businessSegmentationMode }: { plan: YearPlan[]; balanceSheets: BalanceSheetPlan[]; futureCapex: { year: number; value: number }[]; businessSegmentationMode: BusinessSegmentationMode }) {
  ...
  return <section className="financial-diagnostics" aria-label="PL妥当性診断">
    ...
    })}</tr></thead><tbody>{group.rows.map((item) => {
      const itemKey = `${group.title}:${item.name}`;
      const isSelected = itemKey === selected?.key;
      const periodEntries = plan.map((row, index) => item.values(row, index));
      const seriesLabels = Array.from(new Set(periodEntries.flatMap((entries) => entries.map((entry) => entry.label))));
      return <tr className={isSelected ? "diagnostic-selected-row" : undefined} key={item.name}><th>{item.name}<small>{item.unit === "千円/人" ? `${moneyUnit}/人` : item.unit}</small></th><td className="diagnostic-copy">{item.formula}</td><td className="diagnostic-copy">{item.check}</td><td className="diagnostic-sparkline-cell"><DiagnosticSparkline plan={plan} row={item} selected={isSelected} onSelect={() => setSelectedKey(itemKey)} /></td><td className="diagnostic-series-cell"><div className="diagnostic-series-labels">{seriesLabels.map((label) => <span key={label}>{label}</span>)}</div></td>{periodEntries.map((entries, index) => <td className="diagnostic-period-cell" key={plan[index].year}><div className="diagnostic-values diagnostic-values-only">{seriesLabels.map((label) => {
        const entry = entries.find((candidate) => candidate.label === label);
        return <strong key={label}>{entry ? formatted(entry.value, item.unit) : "—"}</strong>;
      })}</div></td>)}</tr>;
...
function OfficialProjectTable({ plan, sourcePlan, drivers }: { plan: YearPlan[]; sourcePlan?: YearPlan[]; drivers: Drivers }) {
  const rows: OfficialRow[] = [
    ...
    { code: "7-20", label: "市場伸び率（年あたり）", unit: "%", value: (_p, i) => i === 0 ? drivers.projectMarketGrowth * 100 : undefined },
      ...
      return <OfficialRowsTable title="補助事業にかかる収支計画" pill="7-1～7-20" plan={plan} sourcePlan={sourcePlan} rows={rows} note="7-10は第6次公式様式に沿った減価償却費合計の入力値です。P2-4・P2-14は、②15指標・目標の会計内訳・利益前提で設定するベース事業の構成比から自動計算します。7-20市場伸び率は、第6次Excelと同じく単一の入力値として最初の列に表示しています。" />;
}
