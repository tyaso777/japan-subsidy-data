...
const driverGroups: { label: string; detail: string; keys: (keyof Drivers)[] }[] = [
  ...
  },
  {
    label: "共通・固定前提",
    detail: "申請・外部前提",
    keys: ["projectMarketGrowth", "subsidy", "localBenchmark"],
  },
];
...
export default function Home() {
  ...
  const achieved = metrics.filter((definition) => targetStatus(definition, actual[definition.key], targets[definition.key]).ok).length;
  ...
  function applyHistoricalDefaults() {
    ...
    nextDrivers.localBenchmark = drivers.localBenchmark || 23;
    ...
    setDefaultNote("すべての計画初期値を設定しました。過去実績が使える項目は平均・変動幅から推計し、実績不足の項目は保守的な補完値を使用しています。未入力の投資額は過去の年平均設備投資額×設備導入年数、補助金額は投資額の3分の1、耐用年数は10年、市場伸び率は5%、ローカルベンチマークは23点で仮置きしています。");
  }
    ...
    <main>
      ...
      {view === "summary" && (
        <section className="page-grid summary-grid">
          <div className="hero-card dark-card">
            ...
            <div className="score-ring"><strong>{achieved}</strong><span>/ 15</span><small>範囲内</small></div>
          </div>
              ...
              {metrics.map((definition) => {
                ...
                return (
                  <div className="metric-row" key={definition.key}>
                    <span className={`status-dot ${status.ok ? "ok" : target.policy === "hard" ? "bad" : "warn"}`} />
                    ...
                    <span className="metric-target">目標 {number(target.value)}{definition.unit}</span>
                  </div>
                );
              })}
            </div>
      ...
      {view === "future" && (
        <section className="content-stack">
          ...
          <div className="workflow-actions"><span>上書きしたセルは固定したまま再最適化できます。調整不要なら、そのまま年度別PLへ進めます。</span><div className="target-action-buttons"><button className="reset-button" onClick={() => setView("pl")}>そのまま年度別PLへ</button><button className="solve-button" onClick={solveAndOpenAnnualPl}>上書き内容を反映して再最適化 →</button></div></div>
        </section>
      )}
      ...
      {view === "targets" && (
        <section className="content-stack">
          ...
          <article className="panel">
            ...
            <div className="wide-table spreadsheet-grid driver-target-table"><table><thead><tr><th>調整項目</th>{historicalPlan.map((row) => <th className="driver-reference-heading" key={row.year}>{row.year}<small>過去実績・参考値<br />{YEAR_ROLE_LABELS[row.role]}</small></th>)}<th>計画初期値</th><th>許容下限</th><th>許容上限</th><th>最適化での扱い</th></tr></thead><tbody>
              {driverGroups.flatMap((group) => [
                ...
                const noRange = key === "investment" || key === "usefulLife";
                const history = historicalDriverSeries[key];
            ...
            <p className="footnote">2023～2025年の各列は、計画値ではなく過去実績の参考値です。現実的な計画初期値・許容範囲を決める材料として表示しています。「過去3期からデフォルト設定」では、補助事業の設備導入期間は過去実績の単純平均を計画初期値、平均±2標準偏差を許容下限・上限とします（技術的な絶対範囲内に制限）。基準年後は、第5次採択者中央値を直接使える項目と、過去採択統計・利益構造から補完する項目を分けています。その他事業は直近を重めにした初期値と過去変動幅から設定します。補助事業投資額と耐用年数は固定入力のため、許容下限・上限を設けません。</p>
            <div className="benchmark-note"><strong>基準年後のデフォルト</strong><span>売上高成長率 22%［15～30%］</span><span>補助事業1人当たり給与支給総額の年平均上昇率 7%［5～10%］</span><span>常時使用する従業員数（就業時間換算）の成長率 4%［0～8%］</span><span>原価率改善 1.5pt［0～2pt］</span><span>その他販管費率 過去平均-1.5pt［過去平均-4～+1pt］</span><span>役員1人当たり給与支給総額の年平均上昇率 7%［5～10%］（従業員と同水準・参考管理）</span><a href="https://chukentou-seichotoushi-hojo.jp/assets/documents/common/5ji_median.pdf" target="_blank" rel="noreferrer">第5次公募・採択者中央値PDF ↗</a></div>
          ...
          <article className="panel table-panel">
            ...
            <p className="footnote">成長率・増加額は2024列と2025列に直前期からの変化を表示します。補助事業売上構成比と投資額比率は、前々期・前期・最新期それぞれの水準です。補助金1円当たり効果とローカルベンチマークは比較可能な過去入力がないため「—」と表示します。</p>
