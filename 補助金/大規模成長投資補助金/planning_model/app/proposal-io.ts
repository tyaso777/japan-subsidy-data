...
export type ProposalExportContext = {
  ...
  metricRows: { label: string; unit: string; actual: number; target: number; max?: number; policy: string }[];
};
...
export function buildProposalHtml({ proposal, effectivePlan, metricRows }: ProposalExportContext) {
  ...
  const metricBody = metricRows.map((row) => `<tr><th>${htmlEscape(row.label)}</th><td>${Number.isFinite(row.actual) ? row.actual.toFixed(2) : "—"} ${htmlEscape(row.unit)}</td><td>${row.target.toFixed(2)} ${htmlEscape(row.unit)}</td><td>${row.max === undefined ? "—" : `${row.max.toFixed(2)} ${htmlEscape(row.unit)}`}</td><td>${htmlEscape(policyLabel(row.policy))}</td></tr>`).join("");
...
export function buildProposalXlsx({ proposal, effectivePlan, metricRows }: ProposalExportContext) {
  ...
  const summaryRows = [
    ...
    rowXml(6, ["15指標・目標", "計画値", "単位", "目標下限", "計画上限", "扱い"], "header"),
    ...metricRows.map((item, index) => rowXml(index + 7, [item.label, item.actual, item.unit, item.target, item.max, policyLabel(item.policy)])),
  ];
  ...
  const assumptionRows = [rowXml(1, ["将来予測・調整水準", "計画初期値", "許容下限", "許容上限"], "title"), rowXml(2, ["調整項目", "計画初期値", "許容下限", "許容上限"], "header"), ...(Object.keys(proposal.drivers) as (keyof Drivers)[]).map((key, index) => { const percent = !["usefulLife", "investment", "subsidy", "localBenchmark"].includes(key); const factor = percent ? 100 : 1; const range = ["investment", "usefulLife"].includes(key) ? undefined : proposal.driverRanges[key]; return rowXml(index + 3, [driverNames[key] ?? key, proposal.drivers[key] * factor, range ? range[0] * factor : undefined, range ? range[1] * factor : undefined]); })];
  ...
  const sheetNames = ["提案計画サマリー", "貸借対照表等", "会社全体PL", "補助事業収支", "妥当性診断", "前提・目標", "モデルデータ"];
  const sheetRows = [summaryRows, reportSheet("1-1～1-25 貸借対照表等（入力結果）", parts.balancePeriods, parts.balanceRows), reportSheet("会社全体にかかる損益計算書（P/L）", parts.periods, parts.companyRows), reportSheet("補助事業にかかる収支計画", parts.periods, parts.projectRows), diagnosticRows, assumptionRows, dataRows];
  const sheetWidths = [[68, 18, 14, 16, 16, 14], [54, ...Array(parts.balancePeriods.length).fill(18)], [54, ...Array(parts.periods.length).fill(18)], [58, ...Array(parts.periods.length).fill(18)], [42, 42, 48, ...Array(parts.periods.length).fill(22)], [68, 18, 18, 18], [120]];
