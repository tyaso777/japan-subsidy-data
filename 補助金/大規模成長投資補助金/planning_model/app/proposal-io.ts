...
export type ProposalExportContext = {
  ...
  metricRows: { key: MetricKey; label: string; unit: string; actual: number; target: number; max?: number; policy: string }[];
};
...
export function buildProposalHtml({ proposal, effectivePlan, metricRows }: ProposalExportContext) {
  ...
  const metricBody = metricRows.map((row) => row.key === "localBenchmark"
    ? `<tr><th>${htmlEscape(row.label)}</th><td>${Number.isFinite(row.actual) ? row.actual.toFixed(2) : "—"} ${htmlEscape(row.unit)}</td><td>—</td><td>—</td><td>固定入力・判定対象外</td></tr>`
    : `<tr><th>${htmlEscape(row.label)}</th><td>${Number.isFinite(row.actual) ? row.actual.toFixed(2) : "—"} ${htmlEscape(row.unit)}</td><td>${row.target.toFixed(2)} ${htmlEscape(row.unit)}</td><td>${row.max === undefined ? "—" : `${row.max.toFixed(2)} ${htmlEscape(row.unit)}`}</td><td>${htmlEscape(policyLabel(row.policy))}</td></tr>`).join("");
  ...
  const driverRows = (Object.keys(proposal.drivers) as (keyof Drivers)[]).filter((key) => key !== "localBenchmark").map((key) => { const percent = !["usefulLife", "investment", "subsidy"].includes(key); const factor = percent ? 100 : 1; const range = ["investment", "usefulLife", "projectMarketGrowth"].includes(key) ? undefined : proposal.driverRanges[key]; return `<tr><th>${htmlEscape(driverNames[key] ?? key)}</th><td>${display(proposal.drivers[key] * factor, percent ? "%" : key === "usefulLife" ? "年" : "億円")}</td><td>${range ? display(range[0] * factor, percent ? "%" : "") : "—"}</td><td>${range ? display(range[1] * factor, percent ? "%" : "") : "—"}</td></tr>`; }).join("");
...
export function buildProposalXlsx({ proposal, effectivePlan, metricRows }: ProposalExportContext) {
  ...
  const summaryRows = [
    ...
    ...metricRows.map((item, index) => rowXml(index + 7, item.key === "localBenchmark" ? [item.label, item.actual, item.unit, undefined, undefined, "固定入力・判定対象外"] : [item.label, item.actual, item.unit, item.target, item.max, policyLabel(item.policy)])),
  ];
  ...
  const assumptionRows = [rowXml(1, ["将来予測・調整水準", "計画初期値", "許容下限", "許容上限"], "title"), rowXml(2, ["調整項目", "計画初期値", "許容下限", "許容上限"], "header"), ...(Object.keys(proposal.drivers) as (keyof Drivers)[]).filter((key) => key !== "localBenchmark").map((key, index) => { const percent = !["usefulLife", "investment", "subsidy"].includes(key); const factor = percent ? 100 : 1; const range = ["investment", "usefulLife", "projectMarketGrowth"].includes(key) ? undefined : proposal.driverRanges[key]; return rowXml(index + 3, [driverNames[key] ?? key, proposal.drivers[key] * factor, range ? range[0] * factor : undefined, range ? range[1] * factor : undefined]); })];
