...
export function buildProposalXlsx({ proposal, effectivePlan, metricRows }: ProposalExportContext) {
  ...
  const summaryRows = [
    ...
    rowXml(6, ["15指標・目標（指標名に第6次定義を併記）", "計画値", "単位", "目標下限", "計画上限", "扱い"], "header"),
    ...metricRows.map((item, index) => { const definedLabel = `${item.label}（第6次定義：${item.round6Formula}）`; return rowXml(index + 7, item.key === "localBenchmark" ? [definedLabel, proposalInput(proposal, inputKey.driver("localBenchmark"), item.actual), item.unit, undefined, undefined, "固定入力・判定対象外"] : isSixthRoundReferenceMetric(item.key) ? [definedLabel, item.actual, item.unit, undefined, undefined, "参考値・第6次評価対象外"] : [definedLabel, item.actual, item.unit, proposalInput(proposal, inputKey.target(item.key, "value"), item.target), proposalInput(proposal, inputKey.target(item.key, "max"), item.max), metricHandling(proposal, item.key, item.policy)]); }),
  ];
