...
\`companyPL\` の将来期は、画面の「将来PLの入力方式（③と共通）」で「全社＋補助事業」か「ベース事業＋補助事業」かを選べます。後者でも、取り込んだ全社値と \`projectPL\` を固定値として保持し、ベース事業は差額として自動計算します。ベース事業の未入力項目と内部明細は、会計前提の変更へ追従します。この選択は③将来データ入力の方式と相互に同期します。
...
- 取込元の数式セルは、保存済み計算結果が古い可能性があるため停止します。入力セルを指定してください。
- 出力先に数式がある場合は停止します。
- 出力後は、Excelで開いたときに数式を全再計算するよう設定します。
- 出力先セルはテンプレート内にあらかじめ存在する必要があります。
- 同じ出力セルを複数定義できません。
- 50MBを超えるExcelや、展開後の内容が過大なExcelは処理しません。
- .xlsmの既存マクロは保持されます。マクロ自体の安全性は検証しないため、信頼できるファイルだけを開いてください。
