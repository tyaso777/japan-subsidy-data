...
export function buildBalanceSheetRows(balanceSheets: BalanceSheetPlan[], plan: YearPlan[]): ReportRow[] {
  ...
  return [
    ...
    { code: "1-11", label: "その他資産（自動計算）", unit: "千円", values: values((row, index) => derived(row, index).otherAssets) },
    ...
    { code: "1-18", label: "その他負債（自動計算）", unit: "千円", values: values((row, index) => derived(row, index).otherLiabilities) },
    ...
    { code: "1-22", label: "その他純資産（自動計算）", unit: "千円", values: values((row, index) => derived(row, index).otherNetAssets) },
