...
export function buildCompanyPlRows(plan: YearPlan[]): ReportRow[] {
  ...
  return [
    ...
    { code: "2-18", label: "経常利益", unit: "億円", values: values((row) => ordinaryIncome(company(row))), emphasis: true },
    { code: "2-19", label: "税引前当期純利益", unit: "億円", values: values((row) => preTaxIncome(company(row))) },
    { code: "2-20", label: "当期純利益", unit: "億円", values: values((row) => netIncome(company(row))) },
...
export function buildProjectPlRows(plan: YearPlan[], marketGrowth: number): ReportRow[] {
  ...
  return [
    ...
    { code: "P2-4", label: "売上原価に含まれる減価償却費（内部管理用）", unit: "億円", values: values((row) => cogsDepreciation(row.project)) },
    { code: "P2-14", label: "販管費に含まれる減価償却費（内部管理用）", unit: "億円", values: values((row) => sgaDepreciation(row.project)) },
    { code: "7-10", label: "減価償却費（合計）", unit: "億円", values: values((row) => cogsDepreciation(row.project) + sgaDepreciation(row.project)) },
