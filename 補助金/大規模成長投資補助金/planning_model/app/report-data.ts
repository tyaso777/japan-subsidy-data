...
export function buildBalanceSheetRows(balanceSheets: BalanceSheetPlan[], plan: YearPlan[]): ReportRow[] {
  ...
  return [
    { code: "1-1", label: "資産総額", unit: "千円", values: values((row) => row.assets), emphasis: true },
    { code: "1-2", label: "うち流動資産", unit: "千円", values: values((row) => row.currentAssets) },
    { code: "1-3", label: "うち現金及び預金", unit: "千円", values: values((row) => row.cash) },
    { code: "1-4", label: "うち固定資産", unit: "千円", values: values((row) => row.fixedAssets) },
    { code: "1-5", label: "うち有形固定資産", unit: "千円", values: values((row) => row.tangibleAssets) },
    { code: "1-6", label: "うち建物及び構築物", unit: "千円", values: values((row) => row.buildings) },
    { code: "1-7", label: "うち機械装置等", unit: "千円", values: values((row) => row.machinery) },
    { code: "1-8", label: "うち土地", unit: "千円", values: values((row) => row.land) },
    { code: "1-9", label: "うち無形固定資産", unit: "千円", values: values((row) => row.intangibleAssets) },
    { code: "1-10", label: "うちソフトウェア", unit: "千円", values: values((row) => row.software) },
    { code: "1-11", label: "その他（流動資産、固定資産を除くその他資産）（自動計算）", unit: "千円", values: values((row, index) => derived(row, index).otherAssets) },
    { code: "1-12", label: "負債及び純資産合計（自動計算）", unit: "千円", values: values((row, index) => derived(row, index).liabilitiesAndNetAssets), emphasis: true },
    { code: "1-13", label: "負債総額", unit: "千円", values: values((row) => row.liabilities) },
    { code: "1-14", label: "うち流動負債", unit: "千円", values: values((row) => row.currentLiabilities) },
    { code: "1-15", label: "うち短期借入金", unit: "千円", values: values((row) => row.shortTermDebt) },
    { code: "1-16", label: "うち固定負債", unit: "千円", values: values((row) => row.fixedLiabilities) },
    { code: "1-17", label: "うち長期借入金", unit: "千円", values: values((row) => row.longTermDebt) },
    { code: "1-18", label: "その他（上記を除く負債）（自動計算）", unit: "千円", values: values((row, index) => derived(row, index).otherLiabilities) },
    { code: "1-19", label: "純資産総額", unit: "千円", values: values((row) => row.netAssets), emphasis: true },
    { code: "1-20", label: "うち株主資本", unit: "千円", values: values((row) => row.shareholderEquity) },
    { code: "1-21", label: "うち資本金", unit: "千円", values: values((row) => row.capital) },
    { code: "1-22", label: "その他（上記を除く純資産）（自動計算）", unit: "千円", values: values((row, index) => derived(row, index).otherNetAssets) },
    { code: "1-23", label: "自己資本比率（自動計算）", unit: "%", values: values((row, index) => derived(row, index).equityRatio) },
    { code: "1-24", label: "新規設備投資による支出", unit: "千円", values: values((row) => row.capex) },
...
export function buildCompanyPlRows(plan: YearPlan[]): ReportRow[] {
  ...
  return [
    { code: "2-1", label: "売上高", unit: "千円", values: values((row) => company(row).sales), emphasis: true },
    { code: "2-2", label: "売上高成長率", unit: "%", values: values((row, index) => growth(company(row).sales, index ? company(plan[index - 1]).sales : undefined)) },
    { code: "2-3", label: "売上原価", unit: "千円", values: values((row) => company(row).cogs) },
    { code: "2-4", label: "うち減価償却費", unit: "千円", values: values((row) => cogsDepreciation(company(row))) },
    { code: "2-5", label: "売上総利益", unit: "千円", values: values((row) => company(row).sales - company(row).cogs), emphasis: true },
    { code: "2-6", label: "売上総利益率", unit: "%", values: values((row) => rate(company(row).sales - company(row).cogs, company(row).sales)) },
    { code: "2-7", label: "販売費及び一般管理費", unit: "千円", values: values((row) => sga(company(row))) },
    { code: "2-8", label: "うち役員の人件費（自動計算）", unit: "千円", values: values((row) => company(row).officerPay) },
    { code: "2-9", label: "うち役員報酬", unit: "千円", values: values((row) => officerCompensation(company(row))) },
    { code: "2-10", label: "うち役員賞与", unit: "千円", values: values((row) => officerBonus(company(row))) },
    { code: "2-11", label: "うち従業員の人件費（自動計算）", unit: "千円", values: values((row) => company(row).employeePay) },
    { code: "2-12", label: "うち従業員の給与", unit: "千円", values: values((row) => employeeSalary(company(row))) },
    { code: "2-13", label: "うち従業員の賞与", unit: "千円", values: values((row) => employeeBonus(company(row))) },
    { code: "2-14", label: "うち減価償却費", unit: "千円", values: values((row) => sgaDepreciation(company(row))) },
    { code: "2-15", label: "うち研究開発費", unit: "千円", values: values((row) => researchDevelopment(company(row))) },
    { code: "2-16", label: "営業利益", unit: "千円", values: values((row) => operatingProfit(company(row))), emphasis: true },
    { code: "2-17", label: "営業利益率", unit: "%", values: values((row) => rate(operatingProfit(company(row)), company(row).sales)) },
    { code: "2-18", label: "経常利益", unit: "千円", values: values((row) => ordinaryIncome(company(row))), emphasis: true },
    { code: "2-19", label: "税引前当期純利益", unit: "千円", values: values((row) => preTaxIncome(company(row))) },
    { code: "2-20", label: "当期純利益", unit: "千円", values: values((row) => netIncome(company(row))) },
    { code: "2-21", label: "給与支給総額（常時使用する従業員）", unit: "千円", values: values((row) => employeePayrollTotal(company(row))) },
    { code: "2-22", label: "給与支給総額（役員）", unit: "千円", values: values((row) => officerPayrollTotal(company(row))) },
    { code: "2-23", label: "減価償却費（合計）", unit: "千円", values: values((row) => company(row).depreciation) },
    { code: "2-24", label: "付加価値額", unit: "千円", values: values((row) => valueAdded(company(row))), emphasis: true },
    ...
    { code: "2-29", label: "従業員1人当たり給与支給総額", unit: "千円/人", values: values((row) => multiple(employeePayrollTotal(company(row)), company(row).headcount)) },
    { code: "2-30", label: "従業員1人当たり給与支給総額の上昇率", unit: "%", values: values((row, index) => growth(multiple(employeePayrollTotal(company(row)), company(row).headcount) ?? 0, index ? multiple(employeePayrollTotal(company(plan[index - 1])), company(plan[index - 1]).headcount) : undefined)) },
    { code: "2-31", label: "役員1人当たり給与支給総額", unit: "千円/人", values: values((row) => multiple(officerPayrollTotal(company(row)), company(row).officerCount)) },
    { code: "2-32", label: "役員1人当たり給与支給総額の上昇率", unit: "%", values: values((row, index) => growth(multiple(officerPayrollTotal(company(row)), company(row).officerCount) ?? 0, index ? multiple(officerPayrollTotal(company(plan[index - 1])), company(plan[index - 1]).officerCount) : undefined)) },
    { code: "2-33", label: "労働生産性", unit: "千円/人", values: values((row) => multiple(valueAdded(company(row)), company(row).headcount + company(row).officerCount)) },
    { code: "2-34", label: "EBITDA", unit: "千円", values: values((row) => ebitda(company(row))), emphasis: true },
...
export function buildProjectPlRows(plan: YearPlan[], marketGrowth: number): ReportRow[] {
  const values = (getter: (row: YearPlan, index: number) => number | undefined) => plan.map(getter);
  return [
    { code: "7-1", label: "売上高", unit: "千円", values: values((row) => row.project.sales), emphasis: true },
    ...
    { code: "7-4", label: "売上総利益", unit: "千円", values: values((row) => row.project.sales - row.project.cogs), emphasis: true },
    { code: "7-5", label: "売上総利益率", unit: "%", values: values((row) => rate(row.project.sales - row.project.cogs, row.project.sales)) },
    { code: "7-6", label: "営業利益", unit: "千円", values: values((row) => operatingProfit(row.project)), emphasis: true },
    { code: "7-7", label: "営業利益率", unit: "%", values: values((row) => rate(operatingProfit(row.project), row.project.sales)) },
    { code: "7-8", label: "給与支給総額（常時使用する従業員）", unit: "千円", values: values((row) => row.project.employeePay) },
    { code: "7-9", label: "給与支給総額（役員）", unit: "千円", values: values((row) => row.project.officerPay) },
    { code: "P2-4", label: "売上原価に含まれる減価償却費（内部管理用）", unit: "千円", values: values((row) => cogsDepreciation(row.project)) },
    { code: "P2-14", label: "販管費に含まれる減価償却費（内部管理用）", unit: "千円", values: values((row) => sgaDepreciation(row.project)) },
    { code: "7-10", label: "減価償却費（合計）", unit: "千円", values: values((row) => cogsDepreciation(row.project) + sgaDepreciation(row.project)) },
    { code: "7-11", label: "付加価値", unit: "千円", values: values((row) => valueAdded(row.project)), emphasis: true },
    ...
    { code: "7-15", label: "従業員1人当たり給与支給総額", unit: "千円/人", values: values((row) => multiple(row.project.employeePay, row.project.headcount)) },
    ...
    { code: "7-17", label: "役員1人当たり給与支給総額", unit: "千円/人", values: values((row) => multiple(row.project.officerPay, row.project.officerCount)) },
    ...
    { code: "7-19", label: "労働生産性", unit: "千円/人", values: values((row) => multiple(valueAdded(row.project), row.project.headcount + row.project.officerCount)) },
...
export function buildDiagnosticGroups(plan: YearPlan[], balanceSheets: BalanceSheetPlan[], futureCapex: { year: number; value: number }[], businessSegmentationMode: BusinessSegmentationMode = "split"): DiagnosticReportGroup[] {
  ...
  const groups = [
    ...
    { title: "2. 人件費・賃上げ", rows: [
      ...
      make("従業員1人当たり給与支給総額", "従業員給与支給総額 ÷ 常時使用する従業員数（就業時間換算）", "給与支給総額の増加が人数増だけになっていないか", "千円/人", (row) => segmentValues(row, payPerEmployee)),
      make("役員1人当たり給与支給総額（参考）", "役員給与支給総額 ÷ 役員数", "役員数の変化を除いた報酬水準が妥当か", "千円/人", (row) => segmentValues(row, (s) => multiple(s.officerPay, s.officerCount))),
    ...
    { title: "3. 生産性", rows: [
      make("従業員1人当たり売上高", "売上高 ÷ 常時使用する従業員数（就業時間換算）", "人員を増やさず売上だけが急増していないか", "千円/人", (row) => segmentValues(row, (s) => perEmployee(s.sales, s))),
      make("1人当たり営業利益", "営業利益 ÷ 常時使用する従業員数（就業時間換算）", "生産性改善が過度になっていないか", "千円/人", (row) => segmentValues(row, (s) => perEmployee(operatingProfit(s), s))),
      make("労働生産性", "付加価値額 ÷（常時使用する従業員数（就業時間換算）＋役員数）", "付加価値・人数・賃上げの関係が整合するか", "千円/人", (row) => segmentValues(row, (s) => multiple(valueAdded(s), s.headcount + s.officerCount))),
    ...
    { title: "5. 補助事業とベース事業の比較", rows: [
      ...
      make("事業別1人当たり売上高", "事業別売上高 ÷ 事業別の常時使用する従業員数（就業時間換算）", "補助事業の生産性だけが突出していないか", "千円/人", (row) => pairedValues(row, (s) => perEmployee(s.sales, s))),
      make("事業別従業員1人当たり給与支給総額", "事業別従業員給与支給総額 ÷ 事業別の常時使用する従業員数（就業時間換算）", "補助事業と既存事業の待遇差が妥当か", "千円/人", (row) => pairedValues(row, payPerEmployee)),
