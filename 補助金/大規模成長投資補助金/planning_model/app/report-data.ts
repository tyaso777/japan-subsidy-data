...
export function buildCompanyPlRows(plan: YearPlan[]): ReportRow[] {
  ...
  return [
    ...
    { code: "2-21", label: "給与支給総額（常時使用する従業員）", unit: "千円", values: values((row) => company(row).employeePay) },
    { code: "2-22", label: "給与支給総額（役員）", unit: "千円", values: values((row) => company(row).officerPay) },
    ...
    { code: "2-29", label: "従業員1人当たり給与支給総額", unit: "千円/人", values: values((row) => multiple(company(row).employeePay, company(row).headcount)) },
    ...
    { code: "2-31", label: "役員1人当たり給与支給総額", unit: "千円/人", values: values((row) => multiple(company(row).officerPay, company(row).officerCount)) },
