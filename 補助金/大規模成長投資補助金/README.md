...
単体配布には `public_metrics_dashboard_online.html` を使用します。企業データと公式統計はHTML内に埋め込まれ、右側の原資料は初期状態で公式Web PDFを参照します。閲覧にはインターネット接続が必要です。「表示」メニューからローカルPDFへ切り替えることもできます。
