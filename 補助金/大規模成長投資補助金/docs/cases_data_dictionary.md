...
本社所在地・事業実施場所の集約列も収録しています。複数所在地を含む明細と出典の扱いは [`case_locations.md`](case_locations.md) を参照してください。
...
`employee_pay_total_base_estimated_oku`、`employee_pay_total_target_estimated_oku`、`employee_pay_total_increase_estimated_oku` は、公開された1人当たり給与と「補助事業に係る従業員数」から作る億円単位の近似値です。`employee_pay_total_calculation_status`、`employee_pay_total_period_alignment`、`employee_pay_total_entity_alignment`、`employee_pay_total_unit_validation`、`employee_pay_total_increase_analysis_status` を必ず併用してください。算式と既知の単位矛盾は [`payroll_total_estimation.md`](payroll_total_estimation.md) に記載しています。
