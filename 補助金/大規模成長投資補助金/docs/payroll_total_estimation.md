...
公開PDFの従業員数は「補助事業に係る従業員数」です。公式の「従業員給与支給総額の増加額」も補助事業に関わる従業員を対象とするため、この対象範囲は整合します。ただし、公開PDFでは「担当役員含む」と記載された人数から従業員数と役員数を分離できない場合があります。公式統計では従業員給与支給総額の増加額（No.10）と役員給与支給総額の増加額（No.12）が別指標であるため、本データのNo.10は原則として近似値です。役員給与分をNo.10に加算するのではなく、従業員分と役員分を分離できないことが主な制約です。
