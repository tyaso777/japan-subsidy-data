...
compatible results, statistically preferred directional results, data-quality
sensitivity, and qualitative review evidence separate.
"""
...
def add_component_wins(cases: pd.DataFrame) -> pd.DataFrame:
    ...
    cases["directional_explanation_level"] = np.select(
        ...
        ["明確な定量補完（2構成以上）", "部分的な定量補完（1構成）"],
        default="公開方向性定量では未説明",
    )
    return cases
...
def main() -> None:
    ...
    cases["dashboard9_applicant_majority"] = cases["dashboard9_at_or_above_applicant_share"].ge(0.50)
    ...
    output_cols = [
        ...
        "dashboard9_at_or_above_applicant_share", "dashboard9_lagging",
        "directional8_observed_n", "directional8_below_accepted_n",
    ...
    summary = {
        ...
        "directional_primary": {
            "definition": "No.13を方向中立として除外した8指標中5指標以上観測、未満60%以上",
