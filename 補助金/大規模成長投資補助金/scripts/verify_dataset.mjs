...
const onlineDashboardHtml = await fs.readFile(path.join(projectDir, "html", "public_metrics_dashboard_online.html"), "utf8");
...
assert(dashboardHtml.includes("data-color-bucket") && dashboardHtml.includes('data-color-bulk="on"') && dashboardHtml.includes('data-color-bulk="off"') && dashboardHtml.includes("function setupColorFilterControls()"), "point-color legend must provide multi-select filters and all-on/all-off controls");
assert(dashboardHtml.includes("colorBaseFiltered") && dashboardHtml.includes("colorFilterValues.has(colorInfo(d).bucket)") && dashboardHtml.includes("表示中 ${selectedCount}/${items.length}区分"), "point-color filters must combine selected color buckets with other filters and show coverage");
...
assert(dashboardHtml.indexOf('<label>縦軸<select id="ymetric"') < dashboardHtml.indexOf('<label>横軸<select id="xmetric"') && dashboardHtml.indexOf('<svg id="histY"') < dashboardHtml.indexOf('<svg id="histX"') && dashboardHtml.indexOf('<h2 id="trendYTitle"') < dashboardHtml.indexOf('<h2 id="trendXTitle"'), "paired axis controls and panels must consistently place the vertical axis on the left and horizontal axis on the right");
...
assert(dashboardHtml.includes('id="selectionList"') && dashboardHtml.includes("data-case-id") && dashboardHtml.includes("activateSelectionCompany") && dashboardHtml.includes("drawDetails();loadPdf()"), "range-selected company list must switch the selected company and PDF");
assert(dashboardHtml.includes("rank=v===null?null:1+peer.filter(x=>x>v).length") && dashboardHtml.includes("`上位${Math.max(1,Math.ceil(rank/peer.length*100))}%`") && dashboardHtml.includes("収録採択企業内 上位割合"), "company detail ranking must display a descending top-percent share");
assert(dashboardHtml.includes('<hr class="filter-divider"><section class="sidebar-companies" id="selectionPanel"') && dashboardHtml.includes('.sidebar-companies{min-width:0;min-height:160px;display:flex;flex:1') && dashboardHtml.includes('.sidebar-companies .selection-list{height:auto;min-height:0;flex:1') && dashboardHtml.includes('id="centerFilterSummary"'), "company list must fill the remaining left-sidebar height and show central filter conditions");
assert(dashboardHtml.includes('<details class="sidebar-filter-box"><summary><span>追加の絞り込み</span>') && !dashboardHtml.includes('<details class="sidebar-filter-box" open>') && dashboardHtml.includes('id="sidebarFilterState"') && dashboardHtml.includes("hiddenFilterCount"), "exclusion, scope, and region filters must be grouped in a default-closed sidebar box with an active-condition indicator");
assert(dashboardHtml.includes("centralFilters={}") && dashboardHtml.includes("function centralFilterRows()") && dashboardHtml.includes("function setCentralFilter(field,lo,hi,source)"), "scatter and histogram metric filters must be stored independently of the selected axes");
assert(!dashboardHtml.includes("[xmetric,ymetric,xlog,ylog].forEach(e=>e.addEventListener('change',()=>clearBoxSelection"), "changing an axis must not clear previously applied central metric filters");
assert(dashboardHtml.includes("function exportRows(){return centralFilterRows()}") && dashboardHtml.includes("'company','pdf_url',xmetric.value"), "CSV export must include PDF URLs and restrict rows to active central metric filters");
assert(dashboardHtml.includes('id="selectionFocus"') && dashboardHtml.includes("e.key!=='ArrowDown'&&e.key!=='ArrowUp'") && dashboardHtml.includes("activateSelectionCompany(next.dataset.caseId,true)"), "selection heading must focus the company list and arrow keys must switch companies");
...
assert(dashboardHtml.includes("const tolerance=Math.max(1,Math.abs(hi-lo))*1e-9,same=histSelection") && dashboardHtml.includes("if(same){delete centralFilters[field];histSelection=null") && dashboardHtml.includes("同じ棒を再クリック：解除"), "clicking the selected histogram bin again must remove only that metric filter");
assert(dashboardHtml.includes("svg.addEventListener('click',e=>{if(e.button!==0||e.target.closest('.bar'))return") && dashboardHtml.includes("delete centralFilters[p.field];if(histSelection?.field===p.field)histSelection=null") && dashboardHtml.includes("空白クリック：この指標の条件解除"), "clicking histogram whitespace must clear only that histogram metric filter");
...
assert(dashboardHtml.includes('id="histXStats"') && dashboardHtml.includes('id="histYStats"') && dashboardHtml.includes("function updateHistogramStats(id,field)") && dashboardHtml.includes("平均 ${fmt(mean)}") && dashboardHtml.includes("分散 ${fmt(variance)}"), "both axis distributions must show filtered count, mean, median, and variance");
assert((dashboardHtml.match(/class="section-box" data-section=/g) || []).length === 6, "all six dashboard content rows must use consistent collapsible section boxes");
assert(dashboardHtml.includes('<section class="section-group" aria-labelledby="scatterGroupTitle">') && dashboardHtml.includes('<h2 class="section-group-title" id="scatterGroupTitle">2軸比較プロット</h2>') && dashboardHtml.includes('data-section="controls" open><summary>表示指標・操作</summary>') && dashboardHtml.includes('data-section="scatter" open><summary>プロット表示</summary>'), "scatter controls and plot must be two independently collapsible rows inside one grouped section");
...
assert(dashboardHtml.includes('data-section="methodology"><summary>公式15指標：算出可否・計算方法・統計量</summary>') && dashboardHtml.includes("const OFFICIAL_METRICS=[") && dashboardHtml.includes("function publicStats(field,round)"), "dashboard must document all 15 official metrics and calculate round-specific public-value statistics");
assert((dashboardHtml.match(/key:'[^']+',label:/g) || []).length === 15 && dashboardHtml.includes("status:'comparable'") && dashboardHtml.includes("status:'proxy'") && dashboardHtml.includes("status:'unavailable'"), "official metric catalog must contain all 15 numbered metrics and comparability classes");
assert(dashboardHtml.includes("母分散（Σ(x－平均)²÷n）") && dashboardHtml.includes("公式側の分散は算出できません") && dashboardHtml.includes("公式代表値は4番だけが平均"), "methodology must distinguish public descriptive statistics from unavailable official variance and mean/median definitions");
...
assert(dashboardHtml.includes('data-table-view="round"') && dashboardHtml.includes('data-table-view="metric"') && dashboardHtml.includes('id="roundFirstView"') && dashboardHtml.includes('id="metricFirstView"'), "comparison statistics must provide round-first and metric-first table views");
assert(dashboardHtml.includes("comparisonHeader(true)") && dashboardHtml.includes('rowspan="4"') && dashboardHtml.includes("analysis-dashboard-table-view-v1"), "metric-first view must group four rounds under each metric and persist the selected table order");
...
for (const [name, document] of [["index.html", html], ["qa.html", qaHtml], ["qa_v0.1.html", qaV01Html], ["public_metrics_dashboard.html", dashboardHtml], ["public_metrics_dashboard_online.html", onlineDashboardHtml]]) {
...
const textFiles = [
  ...
  "docs/validation.md", "docs/analysis_quality_flags.md", "html/index.html", "html/qa.html", "html/qa_v0.1.html", "html/analysis_dashboard.html", "html/public_metrics_dashboard.html", "html/public_metrics_dashboard_online.html", "html/data/cases.json", "scripts/build_dataset.mjs", "scripts/build_qa_v01.py", "scripts/build_analysis_dashboard.py",
