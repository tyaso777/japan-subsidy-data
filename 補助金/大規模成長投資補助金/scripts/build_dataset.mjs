...
async function showPdf(r,page){const token=++pdfRequestToken,pageNo=page||1,localUrl=r.local_pdf+'#page='+pageNo;$('#pdfOpen').href=localUrl;$('#pdfOfficial').href=r.pdf_url+'#page='+pageNo;$('#pdfLabel').textContent=r.company+' / p.'+pageNo;if(location.protocol==='file:'){showPdfNotice('GitHubのZIPにはPDF本体が含まれていません。上の「公式PDFを開く」を利用してください。別途local_assetsへPDFを配置済みの場合は「ローカルPDFを試す」から開けます。');return}showPdfNotice('ローカルPDFを確認しています…');try{const response=await fetch(r.local_pdf,{method:'HEAD',cache:'no-store'});if(token!==pdfRequestToken)return;if(!response.ok)throw new Error(String(response.status));const frame=$('#pdfFrame');$('#pdfNotice').hidden=true;frame.hidden=false;frame.src=localUrl}catch(_){if(token!==pdfRequestToken)return;showPdfNotice('ローカルPDFが見つかりません。GitHubのZIPにはPDF本体が含まれていないため、上の「公式PDFを開く」を利用してください。') }}
