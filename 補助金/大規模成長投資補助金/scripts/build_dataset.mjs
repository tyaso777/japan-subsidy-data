...
function filtered(){const term=query.value.trim().toLowerCase();return DATA.filter(r=>{const rs=reviewState(r.case_id);const hay=[r.company,r.target_entity,r.headline,r.vision,r.external_motivation,r.internal_motivation,r.company_overview,r.other_boxes,...r.boxes.map(b=>b.text)].join('\\n').toLowerCase();return(!round.value||r.round===round.value)&&(!issue.value||(issue.value==='issue'?r.issue_count>0:r.issue_count===0))&&(!review.value||rs.status===review.value)&&(!term||hay.includes(term))})}
