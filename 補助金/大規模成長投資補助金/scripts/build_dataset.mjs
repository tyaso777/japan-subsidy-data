...
import { deriveSalesSeries, validateSalesSeries } from "./sales_series.mjs";
...
const salesSeries = deriveSalesSeries(salesData.rows);
const salesSeriesByCase = new Map();
for (const row of salesSeries) {
  if (!salesSeriesByCase.has(row.case_id)) salesSeriesByCase.set(row.case_id, []);
  salesSeriesByCase.get(row.case_id).push(row);
}

function chooseApplicantSeries(item, rows) {
  const groupApplicant = /グループ|ホールディングス|HOLDINGS|(?:^|\s)HD(?:$|\s)/i.test(`${item.company} ${item.target_entity ?? ""}`);
  const score = (row) => {
    const label = `${row.series_label ?? ""} ${row.scope ?? ""}`;
    let value = 0;
    if (/補助事業|既存事業|親会社|子会社|関連会社|東京連結|コンソーシアム/.test(label)) value -= 200;
    if (/^当社$|当社単体/.test(row.series_label ?? "")) value += 140;
    if (/単体|単独/.test(label)) value += 130;
    if (/会社全体|全社/.test(label)) value += 120;
    if (/代表系列/.test(label)) value += 70;
    if (/連結|グループ/.test(label)) value += groupApplicant ? 80 : -80;
    if (/代表系列/.test(label) && /連結|グループ/.test(item.scope ?? "") && !groupApplicant) value -= 200;
    if (rows.length === 1 && value >= 0) value += 60;
    value += [row.baseline_sales_oku, row.target_sales_oku, row.increase_oku, row.growth_rate_pct, row.cagr_pct].filter((x) => x !== null && x !== undefined && x !== "").length;
    return value;
  };
  const ranked = rows.map((row) => ({ row, score: score(row) })).sort((a, b) => b.score - a.score);
  if (!ranked.length || ranked[0].score < 0) return { row: null, reason: "申請企業自身と判断できる系列なし" };
  const row = ranked[0].row;
  let reason = "申請企業の代表系列";
  if (/単体|単独/.test(row.series_label ?? "")) reason = "単体・単独系列を優先";
  else if (/^当社$|当社単体/.test(row.series_label ?? "")) reason = "当社系列を優先";
  else if (/会社全体|全社/.test(row.series_label ?? "")) reason = "会社全体・全社系列を優先";
  else if (/連結|グループ/.test(row.series_label ?? "") && groupApplicant) reason = "申請者がグループ・持株会社のためグループ系列";
  else if (rows.length === 1) reason = "他系列の記載がないため唯一の系列";
  return { row, reason };
}

const salesSelectionByCase = new Map();
for (const item of review.cases) {
  const rows = salesSeriesByCase.get(item.case_id) ?? [];
  const reported = rows.find((row) => row.is_primary) ?? rows[0] ?? null;
  const applicant = chooseApplicantSeries(item, rows);
  for (const row of rows) {
    row.is_reported_primary = row === reported;
    row.is_applicant_representative = row === applicant.row;
    row.representative_reason = row === applicant.row ? applicant.reason : "";
    row.series_role = row === applicant.row && row === reported ? "申請企業代表・PDF主系列" : row === applicant.row ? "申請企業代表" : row === reported ? "PDF主系列" : "その他系列";
  }
  salesSelectionByCase.set(item.case_id, { reported, applicant: applicant.row, reason: applicant.reason });
}
...
const cases = review.cases.map((item) => {
  ...
  const selection = salesSelectionByCase.get(item.case_id) ?? {};
  const applicantSale = selection.applicant ?? {};
  const reportedSale = selection.reported ?? {};
  ...
  const row = {
    ...
    investment_representative_basis: "同一申請内の投資案件合計",
    ...
    sales_representative_series_id: applicantSale.series_id,
    sales_representative_scope: applicantSale.series_label,
    sales_representative_reason: selection.reason,
    sales_representative_review_required: applicantSale.review_required ?? true,
    sales_baseline_year: applicantSale.baseline_year,
    sales_baseline_oku_yen: applicantSale.baseline_sales_oku,
    sales_target_year: applicantSale.target_year,
    sales_target_oku_yen: applicantSale.target_sales_oku,
    sales_increase_oku_yen: applicantSale.increase_oku,
    sales_growth_pct: applicantSale.growth_rate_pct,
    sales_cagr_pct: applicantSale.cagr_pct,
    sales_multiple: applicantSale.baseline_sales_oku && applicantSale.target_sales_oku ? applicantSale.target_sales_oku / applicantSale.baseline_sales_oku : "",
    sales_validation: applicantSale.arithmetic_status,
    sales_reported_primary_series_id: reportedSale.series_id,
    sales_reported_scope: reportedSale.series_label,
    sales_reported_baseline_year: reportedSale.baseline_year,
    sales_reported_baseline_oku_yen: reportedSale.baseline_sales_oku,
    sales_reported_target_year: reportedSale.target_year,
    sales_reported_target_oku_yen: reportedSale.target_sales_oku,
    sales_reported_increase_oku_yen: reportedSale.increase_oku,
    sales_reported_growth_pct: reportedSale.growth_rate_pct,
    sales_reported_cagr_pct: reportedSale.cagr_pct,
...
const caseColumns = [
  ...
  "investment_representative_basis",
  ...
  "sales_reported_primary_series_id", "sales_reported_scope", "sales_reported_baseline_year", "sales_reported_baseline_oku_yen",
  "sales_reported_target_year", "sales_reported_target_oku_yen", "sales_reported_increase_oku_yen", "sales_reported_growth_pct", "sales_reported_cagr_pct",
...
const salesSeriesColumns = [
  "case_id", "round", "company", "series_id", "series_label", "scope", "is_primary", "is_reported_primary", "is_applicant_representative", "series_role", "representative_reason",
  "baseline_period_label", "baseline_year", "baseline_month", "baseline_period_type", "baseline_year_status", "baseline_sales_oku",
  "target_period_label", "target_year", "target_month", "target_period_type", "target_year_status", "target_sales_oku",
  "increase_oku", "growth_rate_pct", "cagr_pct", "rate_type", "extraction_method", "confidence", "review_required",
  "arithmetic_status", "page", "raw_fragment", "pdf_url",
];
await writeCsv(path.join(out.processed, "sales_series.csv"), salesSeries, salesSeriesColumns);
const salesSeriesValidationIssues = validateSalesSeries(salesSeries);
if (salesSeriesValidationIssues.length) throw new Error(`sales_series validation failed:\n${salesSeriesValidationIssues.join("\n")}`);

const salesSeriesAnnual = [];
for (const row of salesSeries) {
  const points = row._points?.length ? row._points : ["baseline", "target"].map((side) => ({
    period_label: row[`${side}_period_label`], normalized_year: row[`${side}_year`], normalized_month: row[`${side}_month`],
    year_status: row[`${side}_year_status`], sales_oku: row[`${side}_sales_oku`], point_type: side,
  }));
  for (const [pointIndex, point] of points.entries()) {
    if (point.sales_oku === null || point.sales_oku === undefined) continue;
    salesSeriesAnnual.push({
      case_id: row.case_id, round: row.round, company: row.company, series_id: row.series_id,
      series_label: row.series_label, point_type: point.point_type ?? (pointIndex === 0 ? "baseline" : pointIndex === points.length - 1 ? "target" : "intermediate"),
      period_label: point.period_label, year: point.normalized_year, month: point.normalized_month, year_status: point.year_status,
      sales_oku_yen: point.sales_oku, page: row.page, pdf_url: row.pdf_url,
    });
  }
}
await writeCsv(path.join(out.processed, "sales_series_annual.csv"), salesSeriesAnnual, [
  "case_id", "round", "company", "series_id", "series_label", "point_type", "period_label", "year", "month", "year_status", "sales_oku_yen", "page", "pdf_url",
]);
...
const boxes = boxData.rows.map((row) => ({
  ...
  box_theme: row.box_theme ?? row.box_title ?? row.box_label ?? row.box_type,
  box_content: row.box_content ?? row.text,
  bbox_x1: row.bbox?.[0],
  bbox_y1: row.bbox?.[1],
  bbox_x2: row.bbox?.[2],
  bbox_y2: row.bbox?.[3],
...
const htmlCases = review.cases.map((item) => {
  ...
  return {
    ...numeric,
    local_pdf: `../local_assets/pdfs/${localPdfStem(item.case_id)}.pdf`,
...
const qaCases = review.cases.map((item) => {
  ...
  return {
    ...
    sales: sale,
    sales_series: salesSeriesByCase.get(item.case_id) ?? [],
    metrics: metricRows,
...
.layout{display:grid;grid-template-columns:330px minmax(540px,1fr) minmax(480px,.9fr);height:calc(100vh - 104px)}.case-sidebar{min-width:0;min-height:0;display:flex;flex-direction:column;border-right:1px solid var(--line);background:#fff}.case-sort{display:grid;grid-template-columns:auto 1fr 1fr;gap:6px;align-items:center;padding:9px 10px;border-bottom:1px solid var(--line);background:#f7f9fa}.case-sort span{font-size:11px;color:var(--muted)}.case-sort button{padding:6px 7px;border:1px solid var(--line);border-radius:6px;background:#fff;color:var(--navy);cursor:pointer}.case-sort button.active{background:var(--blue);border-color:var(--blue);color:#fff}.case-list{flex:1;min-height:0;overflow:auto}.case{padding:12px 14px;border-bottom:1px solid var(--line);cursor:pointer}.case:hover,.case.active{background:var(--cyan)}.case-head{display:flex;gap:8px;align-items:flex-start}.case-name{font-weight:700;flex:1}.case-id{display:block;margin-top:4px;color:var(--muted);font:11px/1.4 Consolas,"Courier New",monospace;overflow-wrap:anywhere}.pill{font-size:11px;border-radius:999px;padding:2px 7px;background:#e8edf1;white-space:nowrap}.pill.issue{background:#fff0d5;color:var(--warn)}.pill.ok{background:#dff4e8;color:var(--ok)}.case-meta{color:var(--muted);font-size:12px;margin-top:5px}.review-dot{width:8px;height:8px;border-radius:50%;background:#aeb8c1;display:inline-block;margin-right:5px}.review-dot.done{background:var(--ok)}.review-dot.recheck{background:var(--bad)}
...
.box{border-left:4px solid #67a9b6;padding:8px 10px;margin:8px 0;background:#fafcfc}.box-title{display:flex;gap:8px;font-weight:700}.box .raw{max-height:220px}.pdf-pane{border-left:1px solid var(--line);background:#3e4852;display:flex;flex-direction:column}.pdf-bar{display:flex;gap:8px;align-items:center;padding:8px;background:#263747;color:#fff}.pdf-bar span{flex:1}.pdf-bar a{color:#fff}.pdf-pane iframe{border:0;width:100%;height:100%;background:#fff}.pdf-notice{margin:auto 28px;padding:20px;border:1px solid #70808e;border-radius:10px;background:#f7f9fa;color:var(--navy);line-height:1.7;text-align:center}.pdf-notice[hidden],.pdf-pane iframe[hidden]{display:none}.empty{padding:40px;text-align:center;color:var(--muted)}
...
<main class="layout"><aside class="case-sidebar"><div class="case-sort"><span>並び順</span><button id="sortCompany" class="active" type="button">会社名順</button><button id="sortFile" type="button">ファイル名順</button></div><div id="caseList" class="case-list"></div></aside><article id="detail" class="detail"><div class="empty">左の案件を選択してください</div></article><aside class="pdf-pane"><div class="pdf-bar"><span id="pdfLabel">ローカルPDF</span><a id="pdfOpen" target="_blank" rel="noopener">ローカルPDFを試す</a><a id="pdfOfficial" target="_blank" rel="noopener">公式PDFを開く</a></div><div id="pdfNotice" class="pdf-notice" hidden></div><iframe id="pdfFrame" title="ローカルPDF" hidden></iframe></aside></main>
<script>
const DATA=${escapeScriptJson(qaCases)};const STORE='growth-subsidy-qa-v1';const SORT_STORE='growth-subsidy-qa-sort';let selected=location.hash.slice(1)||DATA[0].case_id;let reviews=JSON.parse(localStorage.getItem(STORE)||'{}');let caseSort=localStorage.getItem(SORT_STORE)==='file'?'file':'company';
...
const $=s=>document.querySelector(s);const query=$('#qaQuery'),round=$('#qaRound'),issue=$('#qaIssue'),review=$('#qaReview'),list=$('#caseList'),detail=$('#detail'),sortCompany=$('#sortCompany'),sortFile=$('#sortFile');
...
function filtered(){const terms=query.value.trim().toLowerCase().split(/\\s+/).filter(Boolean);return DATA.filter(r=>{const rs=reviewState(r.case_id);const hay=[r.case_id,r.company,r.target_entity,r.headline,r.vision,r.external_motivation,r.internal_motivation,r.company_overview,r.other_boxes,...r.boxes.map(b=>b.text)].join('\\n').toLowerCase();return(!round.value||r.round===round.value)&&(!issue.value||(issue.value==='issue'?r.issue_count>0:r.issue_count===0))&&(!review.value||rs.status===review.value)&&terms.every(term=>hay.includes(term))})}
...
function fileParts(id){const raw=String(id||'');const seriesMatch=raw.match(/^s(\d+)_/i),numberMatch=raw.match(/(\d+)(?!.*\d)/);return{series:seriesMatch?Number(seriesMatch[1]):Number.MAX_SAFE_INTEGER,number:numberMatch?Number(numberMatch[1]):Number.MAX_SAFE_INTEGER,raw}}
function compareFile(a,b){const x=fileParts(a.case_id),y=fileParts(b.case_id);return x.series-y.series||x.number-y.number||x.raw.localeCompare(y.raw,'ja',{numeric:true})}
function sortedRows(rows){return [...rows].sort(caseSort==='file'?compareFile:(a,b)=>String(a.company||'').localeCompare(String(b.company||''),'ja')||compareFile(a,b))}
function setCaseSort(mode){caseSort=mode==='file'?'file':'company';localStorage.setItem(SORT_STORE,caseSort);sortCompany.classList.toggle('active',caseSort==='company');sortFile.classList.toggle('active',caseSort==='file');renderList()}
function renderList(){const rows=sortedRows(filtered());$('#shown').textContent=rows.length;list.innerHTML=rows.map(r=>{const s=reviewState(r.case_id);return '<div class="case '+(r.case_id===selected?'active':'')+'" data-id="'+esc(r.case_id)+'"><div class="case-head"><span class="pill">'+esc(r.round)+'</span><span class="case-name">'+esc(r.company)+'</span>'+(r.issue_count?'<span class="pill issue">'+r.issue_count+'件</span>':'<span class="pill ok">候補なし</span>')+'</div><span class="case-id">'+esc(r.case_id)+'</span><div class="case-meta"><span class="review-dot '+esc(s.status)+'"></span>'+({done:'確認済',recheck:'要再確認',unreviewed:'未確認'}[s.status])+' ・ '+r.page_count+'頁 ・ 事業費 '+num(r.project_cost_million_yen)+'百万円</div></div>'}).join('')||'<div class="empty">該当案件なし</div>';list.querySelectorAll('.case').forEach(el=>el.onclick=()=>selectCase(el.dataset.id))}
...
function salesSeriesTable(rows){return '<table><thead><tr><th>系列</th><th>基準期（原文／正規化）</th><th>基準売上</th><th>目標期（原文／正規化）</th><th>目標売上</th><th>増加額</th><th>成長率 / CAGR</th><th>検証</th></tr></thead><tbody>'+rows.map(s=>'<tr><td><span class="pill '+(s.is_applicant_representative?'ok':'')+'">'+esc(s.series_role)+'</span><br><b>'+esc(s.series_label)+'</b><br><small>'+esc(s.extraction_method)+'</small></td><td>'+esc(s.baseline_period_label||'記載なし')+'<br><small>'+val(s.baseline_year)+(s.baseline_month?'/'+s.baseline_month:'')+' ・ '+esc(s.baseline_year_status)+'</small></td><td class="num">'+num(s.baseline_sales_oku)+'億円</td><td>'+esc(s.target_period_label||'記載なし')+'<br><small>'+val(s.target_year)+(s.target_month?'/'+s.target_month:'')+' ・ '+esc(s.target_year_status)+'</small></td><td class="num">'+num(s.target_sales_oku)+'億円</td><td class="num">'+num(s.increase_oku)+'億円</td><td class="num">'+num(s.growth_rate_pct)+'%<br><small>CAGR '+num(s.cagr_pct)+'%</small></td><td><span class="pill '+(s.review_required?'issue':'ok')+'">'+(s.review_required?'要確認':esc(s.arithmetic_status))+'</span><br><button class="source" data-page="'+(s.page||1)+'">p.'+(s.page||1)+'</button><details><summary>系列原文</summary><div class="raw">'+esc(s.raw_fragment)+'</div></details></td></tr>').join('')+'</tbody></table>'}
function salesSeriesHtml(rows){const applicant=rows.filter(s=>s.is_applicant_representative),reported=rows.filter(s=>s.is_reported_primary&&!s.is_applicant_representative),others=rows.filter(s=>!s.is_applicant_representative&&!s.is_reported_primary);const group=(title,description,items,empty='')=>'<h4>'+title+'</h4><p style="color:#64717d">'+description+'</p>'+(items.length?salesSeriesTable(items):'<div class="empty">'+empty+'</div>');return group('申請企業自身の代表系列','cases.csv の売上代表値として使用します。単体・当社・会社全体を優先します。',applicant,'売上について、申請企業単体と判定できる系列がありません。労働生産性・給与等に申請企業別の値がある場合でも、売上とは別指標です。')+(reported.length?group('PDF上の主系列','PDFで先頭・主目標として掲げられた系列です。申請企業代表とは限りません。',reported):'')+(others.length?group('その他の系列','連結、親子・関連会社、補助事業、既存事業などの比較用系列です。',others):'')}
function boxHtml(boxes){return boxes.map(b=>'<div class="box"><div class="box-title"><span>p.'+b.page+' / Box '+b.box_no+'</span><span class="pill">'+esc(b.box_label||b.box_type)+'</span><button class="source" data-page="'+b.page+'">PDFで確認</button></div><div><b>枠テーマ：</b>'+esc(b.box_theme||b.box_title||b.box_label||'')+'</div><div class="raw"><b>枠内容：</b>\n'+esc(b.box_content??b.text)+'</div><small>'+esc(b.source_method)+' ・ bbox ['+[b.bbox_x1,b.bbox_y1,b.bbox_x2,b.bbox_y2].map(num).join(', ')+']</small></div>').join('')}
...
[query,round,issue,review].forEach(el=>el.addEventListener('input',renderList));sortCompany.onclick=()=>setCaseSort('company');sortFile.onclick=()=>setCaseSort('file');$('#exportReview').onclick=exportReviews;setCaseSort(caseSort);renderStats();selectCase(selected);
</script></body></html>`;
...
公式中央値の「採択者」と、本データが対象とする「HPで取組概要PDFが公開された交付決定企業」は同一母集団とは限りません。また、同じ指標名でも対象企業、対象範囲、基準年、計画期間が異なる場合があるため、比較時は定義を併記してください。ダッシュボード用に転記した値は \`data/reference/official_round_benchmarks.csv\`、値ごとの出典方針と資料差分は [\`docs/official_round_benchmarks.md\`](docs/official_round_benchmarks.md) にあります。
...
- \`data/processed/unit_revalidation_changes.csv\`: 再換算による金額精緻化を含む変更一覧
...
詳細は \`docs/methodology.md\`、列定義は \`docs/data_dictionary.md\`、単位処理は \`docs/unit_normalization.md\`、検証内容は \`docs/validation.md\` を参照してください。
...
\`local_assets/\` は \`.gitignore\` 対象で、GitHubのZIPにはPDF本体が含まれません。ZIPから \`qa.html\` を開いた場合は右側に案内が表示されるため、［公式PDFを開く］を使用してください。ローカルPDFを準備した環境では、このフォルダのPDFを相対パスで表示します。案件IDと公式URLは元資料名を保持しますが、ローカルPDF名は互換性のため連続アンダースコアを1個へ正規化します（例：案件ID \`s1_outline__179\` → ローカルPDF \`s1_outline_179.pdf\`）。
...
- \`unit_revalidation_changes.csv\`: 単位訂正と再換算による金額精緻化の履歴。

## 原単位と換算値

- 主要指標は \`base_value_raw\`、\`target_value_raw\`、\`unit_raw\` と万円/人換算値を併存させる。
- 事業費・補助額は原値・原単位と \`*_million_yen_normalized\` を併存させる。
- 売上増加額は原値・原単位と \`increase_oku_normalized\` を併存させる。
- 単位は公募回次ではなく、項目ラベル・単位・Box・主体の対応で判定する。
...
- \`*_year\`, \`*_month\`: 分析用の正規化値。2桁年は2000年代と仮定したことを状態列に明示する。
...
生成AIによる画像読解は、\`review_required=true\` の案件を再確認する例外処理・目視検証として用います。
`;
...
const stats = {
  ...
  sales_series: salesSeries.length,
  sales_series_annual: salesSeriesAnnual.length,
  sales_applicant_representatives: salesSeries.filter((row) => row.is_applicant_representative).length,
